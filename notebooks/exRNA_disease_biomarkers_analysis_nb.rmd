...
illumina_read_len <- list.files(file.path(output_data.dir, 'nanopore', 'illumina_read_len'), full.names = T)[1:20]
names(illumina_read_len) <- gsub(basename(illumina_read_len), pattern = '.csv', replacement = '')[1:20]

name_conversion <- data.frame('in_name' = stringr::str_split_fixed(names(illumina_read_len), '[.]', n = 2)[,1])

name_conversion$out_name <- c('panc.1.2.3', 'panc.2.2.7', 'panc.4.3.9', 'panc.5.4.5', 'panc.6.2.5', 'panc.7.2.5',
                              'panc.9.2.5', 'panc.7.1.1', 'panc.9.1.1', 'panc.10.1.2', 'ctrl.12.1.1', 'ctrl.5.1.3',
                              'ctrl.7.1.2', 'ctrl.9.1.2', 'ctrl.13.0.7', 
...
illumina_read_len.list %>%
  ...
  merge(sample_meta_data$`2022.01.14_bioIVTAndPitts_meta_data.csv`, by.x = 'sample_name', by.y = 'patient') %>% 
...
illumina_biotype_to.plt %>% 
  group_by(sample, biolabel, condition) %>% 
  summarize(read_length = mean(abs(read_length))) -> tmp_mean_read_len.df
...
tmp_mean_read_len.df %>%
  ...
  facet_wrap(~biolabel, scales = 'free', nrow = 2, ncol = 2) +
  scale_color_manual(values = viridis_three_color_pal)
