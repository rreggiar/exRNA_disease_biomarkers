...
internal_rmsk_quant$panc_normal$deseq$norm_counts.df %>%
  ...
  # mutate(obs = ifelse(frequency > 0, 1 ,0)) %>%
  # mutate(exp = n(), obs = sum(obs)) %>%
  filter(frequency > 0) %>%
  ...
  # geom_hline(
  #   yintercept = c(500, 1000, 5000, 10000),
  #   linetype = 'dashed',
  #   alpha = 0.4
  # ) +
  geom_boxplot(fill = NA,
               ...
               aes(group = diagnosis),
               show.legend = F) +
  geom_jitter(width = 0.125, show.legend = F) +
  # scale_y_continuous(breaks = c(1000, 5000, 10000)) +
  scale_shape_manual(values = disc_identity_shape_pal) +
  scale_color_manual(values = disc_identity_color_pal) +
  ylab('detected genes') + xlab('') +
  ggforce::facet_row(~ type, strip.position = 'bottom', scales = 'free_y') -> fig.1E.plt
```
### Fig_1_patchwork
```{r Fig1}
...
panc_label_list <- panc_annotated_rmsk_de %>%
  filter(grepl('LTR|^L1|Alu|^SSU', gene)) %>%
...
panc_label_list <-
  rbind(panc_label_list, panc_label_lnc_list_dn)
...
panc_annotated_rmsk_de %>% 
  ...
  ggplot(aes(baseMean, log2FoldChange, color = sig, label = gene)) +
  geom_point() +
  scale_color_grey() +
  ggrepel::geom_text_repel(aes(label = ifelse(sig == T, gene, ''))) +
  ylim(-5, 5)
