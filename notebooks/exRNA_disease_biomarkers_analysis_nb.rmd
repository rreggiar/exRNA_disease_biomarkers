...
internal_gencode_quant$panc_normal$deseq$fgsea <- 
  make.fgsea.ranks(de.seq = 
                     internal_gencode_quant$panc_normal$deseq$de_out)
...
internal_rmsk_quant$panc_normal$deseq$norm_counts.df %>%
  ...
  filter(!gene %in% chrM.filter.list, !grepl('GENCODE', clade)) -> total_annotated_rmsk_counts
...
elife_rmsk_quant$rmsk$quant_meta %>% 
  ...
  ggpubr::stat_compare_means(comparisons = list(c('Repeat-naive', 'Repeat-aware')), 
                           method = 'wilcox', size = txt.mm_to_pts(1.2), 
                           method.args = list(exact = TRUE, paired = T), label = 'p.signif') -> fig.S4B.plt
...
tidy_total_annotated_rmsk_counts %>%
  ...
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) -> fig.S4A.plt
...
wrap_plots(
  fig.S4A.plt,
  guide_area(),
  fig.S4B.plt + coord_cartesian(clip = 'off') + theme(axis.text.x = element_text(angle = 40, hjust = 1)),
...
# write_rds(final_alpha_new, file = file.path(output_data.dir, 'final_alpha_new.rds'))
# 
