...
unique(c(filter(ucsc.rmsk.salmon_quant$analysis_set$covid_ctrl$deseq$de_out, 
                padj < 0.001, !grepl('ENSG', ensg))$ensg,
  filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
         padj < 0.001, !grepl('ENSG', ensg))$ensg)) ->
  de_ensg_names
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
       padj < 0.01, !grepl('ENSG', ensg))$ensg ->
  de_ensg_names
...
y_hat_predictions <- predict(object = binom_model$lambda.min, newx = x_in_filt, newy = y_in, type = 'response')
...
y_hat_predictions <- ifelse(inverse.logit.calc(y_hat_predictions) > perf_list$p.thresh, 1, 0)
...
binomial_model_response_prob %>%
  ...
  annotate(geom = 'text', y = perf_list$p.thresh + 0.025, x = 1.5, 
...
ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out %>% 
  ...
  geom_point() + 
  scale_color_manual(values = te_alone_color_pal) + 
