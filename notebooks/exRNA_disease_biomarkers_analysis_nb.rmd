...
mapping_rate_compare.df %>% 
  select(-condition, -context, -sex) %>% 
  mutate(mapping_rate_diff = `gencode` - `TE-aware`) %>% 
  ...
  ggplot(aes(mapping_rate_diff, star_multimapped_percent, color = condition)) + 
  geom_point() + 
...
save.manuscript.panel(figure = '3',
                      figure.dir = figure.dir,
                      name = 'patchwork_full_biotype_tx_S3',
...
#                       figure.dir = figure.dir,
#                       name = 'patchwork_full_nanopore_biotype_tx_S6',
#                       width = figure_double_column_width,
#                       height = figure_depth,
...
gene_splice_ratio %>% 
  select(sample, gene, unsplice_rate) %>% 
  spread(sample, unsplice_rate) %>%
  select(!contains(qc_fails)) %>% 
  column_to_rownames('gene') %>% 
  t() %>% 
  as.data.frame() -> unsplice_rates_pca.tmp
...
pca_tmp_in_sd <- apply(unsplice_rates_pca.tmp, 2, sd)
pca_tmp_in_zero_sd <- unsplice_rates_pca.tmp[,pca_tmp_in_sd!=0 & !is.na(pca_tmp_in_sd)]
non_zero_pca <- prcomp(pca_tmp_in_zero_sd, center = T, scale. = T, rank. = 50)
...
pca.out %>% 
  ...
  geom_hline(yintercept = 0, linetype = 'dotted', alpha = 0.3, size = 0.25) + 
  scale_color_manual(values = identity_color_pal) + 
  # annotate(geom = 'text', x = -15, y = -160, label = 'process-aware', size = txt.mm_to_pts(1)) +
  theme(legend.position = c(0.98,1), legend.direction = 'horizontal')
```
...
unsplice_counts_pca.df %>% 
  select(!contains(qc_fails)) -> unsplice_filt.df

split.gxi <- process.aware.salmon_quant$analysis_set$ctrl_panc_covid$gxi.split

unsplice.df <- assay(split.gxi, "unspliced") %>% as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  gather(sample, count, -gene) %>% 
  group_by(sample, gene) %>% 
  summarize(unsplice_count = sum(count))

splice.df <- assay(split.gxi, "spliced") %>% as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  gather(sample, count, -gene) %>% 
  group_by(sample,gene) %>% 
  summarize(splice_count = sum(count))

gene_splice_ratio <- 
  merge(unsplice.df, splice.df, by = c("sample", "gene")) %>% 
  mutate(unsplice_rate = unsplice_count/(unsplice_count + splice_count)) %>% 
  drop_na()

gene_splice_ratio %>% 
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene', all.x = T) %>% 
  mutate(family = ifelse(gene %in% 
                           reference_meta_data$gencode_lncRNA_gene_names.df$ensg,
                         'gencode_lncRNA', family),
         family = ifelse(is.na(family), 'gencode_coding', family),
         clade = ifelse(grepl('gencode', family), family, clade),
         clade = ifelse(clade %in% c('gencode_coding',
         ...
         type = ifelse(grepl('gencode',family), 'GENCODE', 'TE'),
         clade = factor(clade, levels = te_aware_annotation_levels)) %>% 
  # gather(sample, unsplice_count, -clade, -family, -type, -gene) %>% 
  merge(sample_meta_data$`2022.01.14_combined_meta_data.csv`, by.x = 'sample', by.y = 'patient') -> gene_splice_ratio_anno

gene_splice_ratio_anno %>% 
  filter(gene %in% filter(salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.01 & log2FoldChange < -1)$ensg) %>% 
  ggplot(aes(clade, unsplice_rate, color = condition)) +
  # geom_histogram(aes(y=stat(density*width)), bins = 100) +
  geom_boxplot() +
  # geom_jitter() +
  # facet_wrap(~clade+condition, ncol = 3) +
  scale_color_manual(values = identity_color_pal)


gene_splice_ratio_anno %>% 
  filter(!condition == 'covid') %>% 
  group_by(gene, condition, clade) %>% 
  summarize(mean_unsplice_rate = mean(unsplice_rate)) %>% 
  merge(salmon_quant$analysis_set$panc_ctrl$deseq$de_out, by.x = 'gene', by.y = 'ensg') %>% 
  spread(condition, mean_unsplice_rate) %>% 
  mutate(delta_unsplice_rate = panc - ctrl) %>% 
  filter(delta_unsplice_rate != 0) %>% 
  ggplot(aes(delta_unsplice_rate, log2FoldChange)) + 
  geom_point() + 
...
process.aware.salmon_quant$analysis_set$ctrl_panc_covid$quant_meta %>% 
  gather(var, value, -sample, -condition, -sex, -context) %>% 
  mutate(value = as.numeric(value),
         condition = factor(condition, levels = c('panc', 'ctrl', 'covid'))) %>% 
  filter(var == 'salmon_percent_mapped') %>% 
  dplyr::rename('Process-aware' = value) %>% 
  merge(tidy_quant_meta.df %>% select(sample, var, value) %>% 
        filter(var == 'salmon_percent_mapped') %>% 
...
mapping_rate_compare.df %>% 
  ...
  ggforce::facet_row(~condition, strip.position = 'bottom') +
  scale_color_manual(values = identity_color_pal) + 
  ggpubr::stat_compare_means(comparisons = list(c('Repeat-aware', 'Process-aware')), 
                             method = 'wilcox', size = 2.5, 
                             method.args = list(exact = TRUE, paired = T))

mapping_rate_compare.df %>% 
  select(-condition, -context, -sex) %>% 
  merge(salmon_quant$analysis_set$ctrl_panc_covid$quant_meta, by = 'sample') %>% 
  mutate(condition = factor(condition, levels = c('panc', 'ctrl', 'covid'))) %>% 
  ggplot(aes(`Repeat-aware` - salmon_percent_mapped, `Process-aware` - salmon_percent_mapped, color = condition)) + 
  geom_point() + 
  ggforce::facet_col(~condition) + 
  geom_smooth(method = 'lm', se = F, linetype = 'dashed') + 
  scale_color_manual(values = identity_color_pal, guide = 'none')
