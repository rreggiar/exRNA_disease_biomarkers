...
pca.out %>% 
  ...
  scale_color_manual(values = disc_identity_color_pal) + ggrepel::geom_text_repel(aes(label = sample))
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  filter(clade == 'SINE') %>% 
  ggplot(aes(salmon_num_mapped, median_sine_abundance, color = condition)) + 
  geom_point()
  # geom_boxplot(fill = NA) + 
  # geom_jitter() +
