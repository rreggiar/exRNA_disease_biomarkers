...
rowRanges(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$txi)[gene_id %in% panc_upreg_alu.df]
...
tx_row_ranges.df %>% 
  filter(gene_id %in% panc_upreg_alu.df$ensg,
         grepl('chr', seqnames)) %>% 
  head(n = 50) -> de_alu_insertion_ranges.df
...
alu_seq_distance <- DECIPHER::DistanceMatrix(alu_seq_aln)
