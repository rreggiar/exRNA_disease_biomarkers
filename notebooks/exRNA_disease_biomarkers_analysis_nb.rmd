...
internal_quant <-
  parse.tximeta.quant.metadata(internal_quant, extra_merge = original_roster_names)
...
SummarizedExperiment::colData(internal_quant$origPanc_normal$gxi)$names
...
external_quant$elife <-
  subset.tximeta.se(external_quant$ucsc_rmsk_quant,
                    filter_list = c(elife_select_list))
```
## gut
```{r}
merge(
...
external_quant$elife$quant_meta <- 
  merge(
    external_quant$elife$quant_meta,
    elife_meta_data,
    by = 'sample'
...
internal_quant$origPanc_normal$deseq <- 
  run.de.seq.individual(input_se = internal_quant$origPanc_normal,
                        type = 'gxi',
                        base_level = 'normal',
                        top_level = 'panc',
                        ref_type = 'ucsc.rmsk',
...
internal_quant$origCovid_normal$deseq <- 
  run.de.seq.individual(input_se = internal_quant$origCovid_normal,
                        type = 'gxi',
                        base_level = 'normal',
                        top_level = 'covid',
                        ref_type = 'ucsc.rmsk',
...
internal_quant$panc_normal$deseq <- 
  run.de.seq.individual(input_se = internal_quant$panc_normal,
                        type = 'gxi',
                        base_level = 'normal',
                        top_level = 'panc',
                        ref_type = 'ucsc.rmsk',
...
internal_quant$esoph_normal$deseq <- 
  run.de.seq.individual(input_se = internal_quant$esoph_normal,
                        type = 'gxi',
                        base_level = 'normal',
                        top_level = 'esoph',
                        ref_type = 'ucsc.rmsk',
...
external_quant$npj$deseq <- 
  run.de.seq.individual(input_se = external_quant$npj,
                        type = 'gxi',
...
external_quant$elife$deseq <- 
  run.de.seq.individual(input_se = external_quant$elife,
                        type = 'gxi',
                        base_level = 'Lung Cancer',
...
```{r}
internal_quant$panc_normal$deseq$norm_counts.df %>% 
  rownames_to_column('gene') %>% 
  # filter(gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  filter(gene %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg) %>%
  mutate(across(where(is.numeric), ~.x/sum(.x))) %>% 
  pivot_longer(names_to = 'sample', values_to = 'frequency', cols = -gene) %>% 
  group_by(sample) %>% 
  filter(frequency > 0) %>% 
  summarize(entropy = -sum((frequency)*log2(frequency))) %>% 
  merge(discovery_meta_data, by = 'sample') %>% 
  # ggplot(aes(diagnosis, entropy)) + 
  # geom_boxplot(fill = NA) +
  # geom_jitter(shape = 21)
  ggplot(aes(entropy, fill = diagnosis)) + 
  geom_density(alpha = 0.3)

internal_quant$origPanc_normal$deseq$norm_counts.df %>% 
  rownames_to_column('gene') %>% 
  # filter(gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  filter(gene %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg) %>%
  mutate(across(where(is.numeric), ~.x/sum(.x))) %>% 
...
external_quant$elife$deseq$norm_counts.df %>% 
  rownames_to_column('gene') %>% 
  # filter(gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  filter(gene %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg) %>%
  mutate(across(where(is.numeric), ~.x/sum(.x))) %>% 
  ...
  filter(frequency > 0) %>% 
  summarize(entropy = -sum((frequency)*log2(frequency))) %>% 
  merge(elife_meta_data, by = 'sample') %>% 
  # ggplot(aes(diagnosis, entropy)) + 
  # geom_boxplot(fill = NA) +
  # geom_jitter(shape = 21)
  ggplot(aes(entropy, fill = diagnosis)) + 
...
external_quant$npj$deseq$norm_counts.df %>% 
  rownames_to_column('gene') %>% 
  # filter(gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  filter(gene %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg) %>%
  mutate(across(where(is.numeric), ~.x/sum(.x))) %>% 
...
total_annotated_rmsk_counts %>% 
  ...
  # mutate(sample = ifelse(is.na(name), sample, name)) %>% 
  # select(-name) %>% 
  merge(external_quant$elife$deseq$scaled_quant_meta_for_de.df %>%
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  merge(external_quant$elife$quant_meta, by = 'sample') %>% 
...
tidy_total_annotated_rmsk_counts %>% 
  group_by(sample, condition, family) %>% 
  filter(!grepl('\\?', family)) %>% 
  summarize(clade_sum = sum(count)) %>% 
  mutate(clade_frac = clade_sum/sum(clade_sum)) %>% 
  merge(external_quant$elife$quant_meta, by = 'sample') %>% 
  ggplot(aes(sample, clade_frac, fill = family)) +
  geom_col() + 
  theme(axis.text.x = element_blank())
