...
qc_fails <- c('panc.6.2.5', 'panc.7.2.5', 'panc.9.2.5', 'panc.10.2.5', 'covid.427.0.80', 'covid.381.0.80')
...
process.aware.salmon_quant$analysis_set$ctrl_panc_covid <- 
    build.analysis.set(se_list = process.aware.salmon_quant, 
                       se_1 = 'bioIvt_covid_process.aware.salmon', 
                       analysis_set_2 = process.aware.salmon_quant$analysis_set$panc_ctrl)
```
...
input_se$quant_meta$condition <- relevel(as.factor(input_se$quant_meta$condition), ref = base_level)

input_se$quant_meta %>% 
  mutate(input_vol = as.numeric(input_vol)) %>% 
  mutate_if(is.numeric, ~scale(., center = T)) %>% 
  mutate_if(is.character, ~as.factor(.)) -> input_se$quant_meta_for_de

print(levels(input_se$quant_meta$condition))

rowRanges(input_se[[type]]) %>% as.data.frame() %>% 
  filter(seqnames %in% c('chrX', 'chrY')) -> sex_filter

rowRanges(input_se[['txi']]) %>% as.data.frame() %>% View()

count_matrix.df <- assay(input_se[[type]], 'counts') %>% 
  as.data.frame() %>% 
  rownames_to_column('gene') %>% 
  mutate_if(is.numeric, round) %>% 
  column_to_rownames('gene')

# dds_formula <- as.formula(~diabetes+age+input_vol+condition)
dds_formula <- as.formula(~age+input_vol+condition)

input_set_dds <- DESeqDataSetFromMatrix(countData = count_matrix.df, 
                                        colData = input_se$quant_meta_for_de, 
                                        design = dds_formula)

input_set_dds <- estimateSizeFactors(input_set_dds)

input_set_dds_norm_counts <- as.data.frame(counts(input_set_dds, normalized=T))

input_set_dds_norm_counts %>% 
  ...
  drop_na() %>% 
  filter(abs(age_cor) >= 0.70) -> condition_aware_age_cor_filter
...
input_set_dds_vst_counts[! rownames(input_set_dds_vst_counts) %in% 
                           c(condition_aware_age_cor_filter$ensg,
                             sex_filter$gene_id), ] %>% t() -> pca_tmp_in 
...
pca.out <- as.data.frame(non_zero_pca$x) %>% 
  ...
  merge(input_se$quant_meta_for_de, by = 'sample')
