...
panc_nanopore_read_length.df %>% 
  ...
  ylab('proportion') + xlab('fragment length') +
  theme(legend.position = c(0.99,0.17)) #-> plt_NA.plt
...
len_compare.df %>%
  ...
  group_by(biotype.x, condition, sample) %>% 
  summarize(read_length = mean(read_length)) %>% 
  merge(nanopore_biotype_labels, by.x = 'biotype.x', by.y = 'biotype') %>% 
  mutate(biolabel = factor(biolabel, levels = te_aware_annotation_levels)) %>% 
  # filter(abs(read_length) > 75) %>% 
  group_by(biolabel, condition) %>%
  filter(! biolabel %in% c('polymorphic_pseudogene', 'TR_C_gene', 'TR_V_gene', 'IG_V_gene', 'IG_C_gene')) %>% 
  ggplot(aes(condition, read_length, color = condition)) +
  geom_boxplot(outlier.colour = NA) + 
  geom_jitter(width = 0.24, alpha = 0.2) +
  facet_wrap(~biolabel, scales = 'free') + #, nrow = 3, ncol = 3) +
  scale_color_manual(values = viridis_three_color_pal, guide = 'none') + 
  ggpubr::stat_compare_means(method = 'wilcox', comparisons = list(c('panc', 'ctrl'))) +
  coord_cartesian(clip = 'off') + 
  xlab('') + ylab('fragment length') +
  theme(legend.position = c(0.99, 0.2), legend.direction = 'horizontal')
...
len_compare.df %>% 
  ...
  ggplot(aes(read_length, len, fill = log1p(..count..))) + 
  # geom_point(alpha = 0.2) + 
  geom_hex(bins = 120) +
  # ylim(0, 500) + xlim(0,400) +
  ...
  # scale_fill_manual(values = RColorBrewer::brewer.pal(3, 'Dark2')) + 
  theme(legend.position = c(0.95,0.10), legend.direction = 'horizontal')# -> plt_NB.plt
...
tmp_mean_read_len.df %>%
  ...
  geom_boxplot() + 
  geom_jitter(width = 0.24, alpha = 0.2) +
  facet_wrap(~biotype, scales = 'free') + #, nrow = 3, ncol = 3) +
  scale_color_manual(values = viridis_three_color_pal, guide = 'none') + 
  ggpubr::stat_compare_means(method = 'wilcox', comparisons = list(c('covid', 'ctrl'), c('panc', 'ctrl'))) +
  coord_cartesian(clip = 'off') + 
  xlab('') + ylab('fragment length') +
  theme(legend.position = c(0.99, 0.2), legend.direction = 'horizontal') -> plt_NC.plt
...
tmp %>% 
  ggplot(aes(read_length, len, color = condition, fill = ..count..)) + 
    geom_bin_2d(bins = 100) +
    ylim(0, 500) + xlim(0,400) +
    xlab('avg. observed fragment length') + ylab('expected TE insertion length') +
...
tmp %>% 
  ...
  ggforce::facet_col(~condition) +
  scale_fill_viridis_c(option = 'magma', direction = -1) +
  theme(legend.position = c(0.95,0.10), legend.direction = 'horizontal')
...
pca.out %>% 
  ...
  scale_color_manual(values = identity_color_pal) + 
  theme(legend.position = c(0.95,1), legend.direction = 'horizontal') -> plt_ND.plt
```
...
wrap_plots(plt_NC.plt,
  ...
  plot_annotation(tag_levels = 'A') -> fig.N.patchwork
