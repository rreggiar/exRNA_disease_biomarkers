...
```{r}
tmp <- read_csv(file.path(output_data.dir, 'lnc.merged_species_TE_df.TEBAG.csv'))
...
tmp %>% 
  ...
  select(insertion = insertion_y, Human, Chimp, Gorilla, Orangutang, Bonobo, `panc.1.2.3`:`ctrl.9.1.2`) %>% 
  mutate(Chimp = as.logical(Chimp),
         Gorilla = as.logical(Gorilla),
         Orangutang = as.logical(Orangutang),
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  mutate(clade_frac = clade_sum/sum(clade_sum),
         condition = factor(condition, levels = c('panc', 'ctrl', 'covid'))) %>%
  mutate(rep = as.numeric(str_split_fixed(string = name, pattern = ' ', n = 2)[,2])) %>% 
  ggplot(aes(reorder(name, rep), clade_frac, fill = clade)) +
...
gencode_layer.plt + 
  ...
  ggrepel::geom_text_repel(data = panc_annotated_rmsk_de %>% 
               ...
               max.overlaps = 1000, box.padding = 0.3, segment.colour = 'grey') -> fig.2B.plt
