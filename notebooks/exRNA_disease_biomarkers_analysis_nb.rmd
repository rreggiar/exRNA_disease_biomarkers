...
## Fig_1stats
```{r}
SummarizedExperiment::assay(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$txi, 'counts') %>% 
  as.data.frame() -> ucsc_rmsk_counts

ucsc_rmsk_counts <- ucsc_rmsk_counts[, !colnames(ucsc_rmsk_counts) %in% qc_fails]

ucsc_rmsk_counts %>%
  rownames_to_column('enst') %>% 
  merge(reference_meta_data$total_tx_to_gene.df, by = 'enst') -> ucsc_rmsk_counts
