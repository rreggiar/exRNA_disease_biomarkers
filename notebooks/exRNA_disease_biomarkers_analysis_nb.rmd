...
tcga_paad_meta_data <- tibble('sample' = rownames(tcga_paad_te_counts), 'condition' = ifelse(grepl('PAAD', sample), 'PAAD', 'GTEx'))
...
tcga_colData %>% 
  ...
  filter(type == 'Primary Tumor') -> tcga_primary_tumors
...
tcga_padd_stratified_counts %>% 
  ...
  filter(gene %in% filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.05, log2FoldChange > .75, !grepl('ENSG', ensg))$gene) %>%
  select(sample, count) %>% 
  group_by(sample) %>%
...
## RERWIP dl survival data for PAAD from Xena
