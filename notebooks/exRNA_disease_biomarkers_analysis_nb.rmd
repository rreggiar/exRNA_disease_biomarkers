...
build_ip_tables <- function(analysis_set,
  ...
  rowRanges(analysis_set$gxi) %>% 
    ...
    select(gene, chr = seqnames, start, end, width, baseMean, padj, log2FoldChange) -> gencode_ip_table.tmp
  ...
  rmsk_enrich_1 %>% 
    ...
    select(gene, tx_name, chr, start, end, width, baseMean, padj, log2FoldChange) -> rmsk_ip_table.tmp
