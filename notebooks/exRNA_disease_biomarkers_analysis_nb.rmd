...
input_de <- ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq

input_de$vst_counts.df[!rownames(input_de$vst_counts.df) %in%
                           c(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq$sex_filter.df$group_name), ] %>%
    t() -> pca_tmp_in
...
## Fig_2A
```{r}
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq$norm_counts.df %>% 
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  merge(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>% select(-dataset), by = 'sample') %>% 
  filter(clade == 'LINE') %>% 
