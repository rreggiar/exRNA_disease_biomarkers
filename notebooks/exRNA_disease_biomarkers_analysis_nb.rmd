...
fig.1C.plt <- 
  ...
  annotate(geom = 'text', x = 125, y = -175, label = 'Repeat-aware', size = txt.mm_to_pts(1.25))
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                        padj < 0.05, log2FoldChange > 0.75) %>%
  filter(!gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  top_n(wt =-padj, 10) %>% dplyr::pull(gene) -> this_top_10
...
# hm_meta_data <- ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$scaled_quant_meta_for_de.df

hm_meta_data <- ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$scaled_quant_meta_for_de.df[match(rownames(hm_count_data),
                                                                                                                                                                                                        ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$scaled_quant_meta_for_de.df$sample), ]
...
ComplexHeatmap::Heatmap(hm_count_data, 
                        show_row_names = F, 
                        show_column_names = T, 
                        heatmap_width = figure_single_column_width,
                        left_annotation = left_hma, 
...
wrap_plots(plt_surv_A.plt,
           guide_area(),
           patchwork::wrap_elements(full = grid::grid.grabExpr(ComplexHeatmap::draw(plt_surv_A.chm, merge_legends = T), 
                                                    width = figure_single_column_width)),
           plt_surv_B.plt,
           patchwork::wrap_elements(full = grid::grid.grabExpr(ComplexHeatmap::draw(plt_surv_B.chm, merge_legends = T), 
                                                    width = figure_single_column_width)),
           plt_surv_C.plt,
           patchwork::wrap_elements(full = grid::grid.grabExpr(ComplexHeatmap::draw(plt_surv_C.chm, merge_legends = T), 
                                                    width = figure_single_column_width)),
           plt_surv_D.plt,
           patchwork::wrap_elements(full = grid::grid.grabExpr(ComplexHeatmap::draw(plt_surv_D.chm, merge_legends = T), 
                                                    width = figure_single_column_width)),
           heights = c(1.25,0.1,0.9,0.9,0.9),
           widths = c(0.75, 1.25),
           guides = 'collect',
...
wrap_plots(patchwork::wrap_elements(fig.S2A.fgsea_table.grob, ignore_tag = F),
           ...
           design = fig_S2_layout, 
           guides = 'collect',
           heights = c(0.6,0.9,0.0937,1,1)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
        legend.title = element_text(vjust = 0.5),
