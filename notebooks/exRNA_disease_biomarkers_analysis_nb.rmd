...
## Fig_flowchart
```{r}
flow_chart.plt <- magick::image_read_pdf(path = file.path(figure.dir, 'fig.1', 'ip_flowchart.pdf'))
...
wrap_plots(fig.1A_1.plt + coord_cartesian(clip = 'off'),
           fig.1A_2.plt + coord_cartesian(clip = 'off'),
           ...
           design = fig_1_layout, 
           guides = 'collect',
           heights = c(1.5,0.65,0.65,0.0625,0.45)) & 
...
tmp_mean_read_len.df %>%
  ...
  mutate(biotype = factor(biotype, levels = c('SINE', 'LINE', 'LTR', 'gencode_lncRNA', 'retained_intron', 'TEC'))) %>% 
  ...
  xlab('') + ylab('median fragment length') -> plt_NA.plt
```
## Fig_NB
```{r}
tmp_mean_read_len.df %>% 
...
panc_nanopore_read_length.df %>% 
  ...
  mutate(biolabel = factor(biolabel, levels = te_aware_annotation_levels),
         sample = case_when(sample == 'panc.7.1.1' ~ 'panc 7', sample == 'panc.6.1.1' ~ 'panc 6')) %>% 
...
illumina_biotype_to.plt %>% 
  filter(biotype == 'SINE',
         sample_name %in% c('panc.6.1.1', 'panc.7.1.1')) %>% 
...
illumina_read_len.df %>% 
  ...
  group_by(sample, biotype, bin, condition) %>%
  summarize(total = n()) %>% 
  pivot_wider(names_from = 'bin', values_from = 'total') %>% 
  ggplot(aes(condition, large/small)) + 
  geom_boxplot(outlier.colour = NA) + 
  geom_jitter(width = 0.24, alpha = 0.2) +
  facet_wrap(~biotype, scales = 'free', ncol = 3, strip.position = 'bottom') +
  scale_color_manual(values = identity_color_pal, guide = 'none') + 
...
panc.6.1.1_hex_to.plt %>% 
  ...
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'panc 6') +
...
panc.7.1.1_hex_to.plt %>% 
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'panc 7') +
...
panc.6.1.1_nanopore_hex_to.plt %>% 
  ...
  annotate(geom = 'text', x = 275, y = 420, label = 'panc 6') +
  xlab('avg. observed fragment length') + ylab('exp. insertion length') +
  # ggforce::facet_col(~sample) +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', breaks = c(0,1)) +
  scale_y_continuous(breaks = c(100,200,300,400), limits = c(75,450)) +
  scale_x_continuous(breaks = c(100,200,300,400), limits = c(75,450)) +
  theme(legend.position = c(0.98,0.35), legend.direction = 'horizontal', legend.key.width = unit(1, 'mm')) +
  guides(fill = guide_colorbar(title.position = 'left', title.vjust = 1)) -> plt_NE_2.plt
...
panc.7.1.1_nanopore_hex_to.plt %>% 
  ...
  annotate(geom = 'text', x = 275, y = 420, label = 'panc 7') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  # ggforce::facet_col(~sample) +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', guide = 'none') +
  scale_y_continuous(breaks = c(100,200,300,400), limits = c(75,450)) +
  scale_x_continuous(breaks = c(100,200,300,400), limits = c(75,450)) -> plt_NE_3.plt

p4 <- ggplot(data.frame(l = plt_NE_3.plt$labels$y, x = 1, y = 1)) +
...
wrap_plots(plt_NE_2.plt + ylab('') + xlab(''),
           plt_NE_3.plt + ylab(''),
           p4,
           widths = c(0.02,1),
...
tmp_mean_read_len.df %>%
  ...
  coord_cartesian(clip = 'off') + 
  theme(strip.placement = 'inside') +
  xlab('') + ylab('median fragment length') -> plt_SNB.plt
```
## Fig_SNC
```{r}
tmp_mean_read_len.df %>% 
...
pca.out %>% 
  ...
  theme(legend.position ='top', legend.direction = 'horizontal') -> plt_SNC.plt
```
...
count_matrix.df[, 'panc.7.1.1', drop = F] %>% rownames_to_column('Gene ID') %>% 
  ...
  group_by(`Gene ID`) %>% 
  summarize(panc.7.1.1_tpm = mean(panc.7.1.1_tpm),
            panc.7.1.1_nanopore_tpm = mean(panc.7.1.1_nanopore_tpm)) %>% 
...
count_matrix.df[, 'panc.6.1.1', drop = F] %>% rownames_to_column('Gene ID') %>%
  ...
  group_by(`Gene ID`) %>% 
  summarize(panc.6.1.1_tpm = mean(panc.6.1.1_tpm),
            panc.6.1.1_nanopore_tpm = mean(panc.6.1.1_nanopore_tpm)) %>% 
...
lm(panc.6.1.1_tpm ~ panc.7.1.1_tpm, data = panc_compare_annotated.df) %>% 
  summary() -> panc_lm_out
...
panc_compare_annotated.df %>% 
  ggplot(aes(log2(panc.6.1.1_tpm + 1), log2(panc.7.1.1_tpm + 1), color = clade, fill = clade)) + 
...
lm(panc.7.1.1_nanopore_tpm ~ panc.7.1.1_tpm, data = panc_compare_annotated.df) %>% 
  summary() -> panc_lm_out
...
panc_compare_annotated.df %>% 
  ggplot(aes(log2(panc.7.1.1_nanopore_tpm + 1), log2(panc.7.1.1_tpm + 1), color = clade, fill = clade)) + 
  geom_hex(alpha = 0.4, bins = 250) +
...
lm(panc.6.1.1_nanopore_tpm ~ panc.6.1.1_tpm, data = panc_compare_annotated.df) %>% 
  summary() -> panc_lm_out
...
panc_compare_annotated.df %>% 
  ggplot(aes(log2(panc.6.1.1_nanopore_tpm + 1), log2(panc.6.1.1_tpm + 1), color = clade, fill = clade)) + 
  geom_hex(alpha = 0.4, bins = 250) +
  annotate(geom = 'text', x = 6, y = 16, fontface = 'italic',
         label = paste0('y', ' ', ' = ',b,' + ',m,'x; adj. R2 = ', r2), 
         size = rel(3.75), alpha = 0.8) +
...
lm(panc.6.1.1_nanopore_tpm ~ panc.7.1.1_nanopore_tpm, data = panc_compare_annotated.df) %>% 
  summary() -> panc_lm_out
...
panc_compare_annotated.df %>% 
  ggplot(aes(log2(panc.6.1.1_nanopore_tpm + 1), log2(panc.7.1.1_nanopore_tpm + 1), color = clade, fill = clade)) + 
  geom_hex(alpha = 0.4, bins = 250) +
  annotate(geom = 'text', x = 3, y = 15, fontface = 'italic',
         label = paste0('y', ' ', ' = ',b,' + ',m,'x; adj. R2 = ', r2), 
         size = rel(3.75), alpha = 0.8) +
...
process.aware.salmon_quant$analysis_set$ctrl_panc_covid$unsplice_data.df %>% 
  ...
  xlab('') + ylab('unsplice rate') -> plt_4A.plt
```
## Fig_4B
```{r}
