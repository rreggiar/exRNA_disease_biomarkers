...
mapping_rate_compare.df %>% 
  ...
  mutate(mapping_rate_diff = `TE-aware` - `gencode`) %>% 
  ...
  ggplot(aes(`TE-aware`, star_multimapped_percent, color = condition)) + 
  geom_point() + 
...
mapping_rate_compare.df %>% 
  ...
  ggforce::facet_row(~condition, strip.position = 'bottom') +
  scale_color_manual(values = identity_color_pal) + 
  ggpubr::stat_compare_means(comparisons = list(c('gencode', 'Process-aware')), 
                             method = 'wilcox', size = 2.5, 
