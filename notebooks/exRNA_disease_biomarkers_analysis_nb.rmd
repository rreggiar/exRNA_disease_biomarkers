...
## Fig_flowchart
```{r}
flow_chart.plt <- magick::image_read_pdf(path = file.path(figure.dir, 'fig.1', 'plasma_schematic.pdf'))
...
wrap_plots(wrap_plots(flowchart_wrap, fig.2C.plt, widths = c(0.25, 0.75)),
           ...
           design = fig_2_layout, 
           guides = 'collect',
           heights = c(1,0.0937,0.75,1)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
...
wrap_plots(plt_SN2B.plt + ggtitle('panc 7 nanopore v nanopore'),
           guide_area(),
           # plt_SN2B.plt + ggtitle('nanopore v illumina'),
           plt_SN2C.plt + ggtitle('panc 6 nanopore v nanopore'),
           plt_SN2D.plt + ggtitle('ctrl 6 nanopore v nanopore'),
           # plt_SN2E.plt,
           guides = 'collect',
...
unique(c(filter(ucsc.rmsk.salmon_quant$analysis_set$covid_ctrl$deseq$de_out, 
                padj < 0.001 & abs(log2FoldChange) > 2.75)$ensg,
  filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
         padj < 0.001 & abs(log2FoldChange) > 0.75)$ensg)) ->
  de_ensg_names
...
lapply(seq(0,1,0.01), function(alpha) {
  set.seed(123)
...
multi_roc <- 
  pROC::multiclass.roc(response = y, 
                       predictor = as.numeric(y_hat_predictions), 
                       levels = c('0','1','2'))
...
beta %>% as.matrix() %>% as.data.frame() %>% 
  rownames_to_column('ensg') %>% 
  merge(ucsc.rmsk.salmon_quant$analysis_set$covid_ctrl$deseq$de_out, by = 'ensg')
```
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
       padj < 0.01 & abs(log2FoldChange) > 0.55 & !grepl('ENSG', ensg))$ensg ->
  de_ensg_names
...
coef_tmp <- cbind(coef(binom_model, s = binom_model$lambda.1se))

beta <- coef_tmp[apply(coef_tmp != 0, 1, any), ]

colnames(beta) <- names(coef(binom_model, s = binom_model$lambda.1se))
...
# predict(tcga_binomial_model, s = 'lambda.min', newx = x_in_train, type = 'response')

training_pred_matrix <- predict(binom_model, s = 'lambda.min', newx = x_in_filt, type = 'response')

plot(pROC::roc(response = y, predictor = training_pred_matrix[,1]))

tcga_binom_test_data <- tcga_x_in[, colnames(tcga_x_in) %in% colnames(x_in_filt)]

testing_pred_matrix <- predict(binom_model, s = 'lambda.min', newx = tcga_binom_test_data, type = 'response')

pROC::roc(response = tcga_y_in %>% pull(condition), predictor = testing_pred_matrix[,1])
...
binomial_model_response_prob %>% 
  ...
  mutate(condition = ifelse(condition == 1, 'PAAD', 'CTRL')) %>% 
...
tcga_padd_stratified_counts %>% 
  ...
  mutate(gene.set = 'panc_te_signal') %>%
  group_by(gene.set) %>%
...
surv.fit <- survfit(as.formula(surv.obj ~ panc_te_signal), 
...
paste0('panc gencode up: ',filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
...
paste0('panc TE down: ',filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
...
paste0('Ox. Phos: ',filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.05, log2FoldChange > 0.75) %>%
...
paste0('MYC Targets: ',filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.05, log2FoldChange > 0.75) %>%
