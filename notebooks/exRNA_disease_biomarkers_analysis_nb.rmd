...
ctrl_hex_to.plt %>% 
  ...
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'ctrl 1') +
...
tmp %>% 
  filter(sample_name == 'panc.9.1.1') -> panc.7.1.1_hex_to.plt

panc.7.1.1_hex_to.plt %>% 
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'panc 1') +
...
tmp %>%
  filter(sample_name == 'panc.7.1.1') -> panc.6.1.1_hex_to.plt

panc.6.1.1_hex_to.plt %>% 
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'panc 2') +
...
ctrl.6.1.5_nanopore_hex_to.plt %>% 
  ...
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 275, y = 175, label = 'ctrl 1') +
...
panc.6.1.1_nanopore_hex_to.plt %>% 
  ...
  annotate(geom = 'text', x = 275, y = 175, label = 'panc 1') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  # ggforce::facet_col(~sample) +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', breaks = c(0,1)) +
  scale_y_continuous(breaks = c(50,100,200,300,400), limits = c(0,400)) +
...
panc.7.1.1_nanopore_hex_to.plt %>% 
  ...
  annotate(geom = 'text', x = 275, y = 175, label = 'panc 2') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  # ggforce::facet_col(~sample) +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', guide = 'none') +
  scale_y_continuous(breaks = c(50,100,200,300,400), limits = c(0,400)) +
...
tmp_mean_read_len.df %>%
  ...
  coord_cartesian(clip = 'off') + 
  theme(strip.placement = 'inside') +
  xlab('') + ylab('median insert length') -> plt_SNB.plt
```
## Fig_SNC
```{r}
tmp_mean_read_len.df %>% 
...
pca.out %>% 
  ...
  theme(legend.position = c(0.95,1), legend.direction = 'horizontal') -> plt_SNC.plt
```
...
wrap_plots(wrap_elements(full = panel.SNA.patchwork),
  ...
  plot_annotation(tag_levels = 'A') &
  theme(legend.justification = 'center', legend.position = 'bottom') -> fig.SN.patchwork
