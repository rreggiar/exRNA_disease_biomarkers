...
lapply(panc_upreg_alu.df$ensg[1:2], function(alu) {
...
txi_tmp <- txi_rr[txi_rr$gene_id == 'AluYa5', ]
...
SummarizedExperiment::elementMetadata(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$txi)[gene_id %in% panc_upreg_alu.df]
  as.data.frame() %>% 
...
alu_seq_distance <- DECIPHER::DistanceMatrix(c(alu_seq_cons.lst[[1]], alu_seq_cons.lst[[2]]))
...
DECIPHER::BrowseSeqs(c(alu_seq_cons.lst[[1]], alu_seq_cons.lst[[2]]))
