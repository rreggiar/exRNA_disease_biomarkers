...
ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out %>% 
  filter(grepl('Alu', ensg), log2FoldChange > 1) %>% 
  mutate(condition = 'panc') -> panc_upreg_alu.df

rowRanges(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$txi) %>% 
  as.data.frame() %>% 
  filter(grepl('^chr', seqnames)) -> txi_rr

# foreach::foreach(alu = panc_upreg_alu.df$ensg, .combine = rbind) %dopar% {
#   
#   txi_tmp <- txi_rr[txi_rr$gene_id == alu, ]
#   
#   # message(alu)
#   
#   de_alu_gr <- makeGRangesFromDataFrame(head(txi_tmp), keep.extra.columns = T)
#   
#   # message('getting seq...')
#   
#   de_alu_seq <- Biostrings::getSeq(BSgenome.Hsapiens.UCSC.hg38, de_alu_gr)
# 
#   # message('aligning seqs.....')
#   
#   alu_seq_aln <- DECIPHER::AlignSeqs(de_alu_seq, processors = 5)
#   
#   # message('grabbing consesnsus......')
#   
#   alu_seq_cons <- DECIPHER::ConsensusSequence(alu_seq_aln)
#   
#   return(alu_seq_cons)
#   
# } -> alu_cons_list

lapply(panc_upreg_alu.df$ensg, function(alu) {
  
  txi_tmp <- txi_rr[txi_rr$gene_id == alu, ]
  message(alu)
  
  de_alu_gr <- makeGRangesFromDataFrame(txi_tmp, keep.extra.columns = T)
  message('getting seq...')
...
lapply(alu_seq_aln.lst, function(alu_seq_aln) {
...
kmer::kdistance(DECIPHER::RemoveGaps(alu_seq_aln.lst[[1]]))

tmp_no_gap <- ape::as.DNAbin(DECIPHER::RemoveGaps(do.call(c, alu_seq_cons.lst)))

tmp_dist <- kmer::kdistance(tmp_no_gap, residues = "DNA", k = 12)

tmp_count <- kmer::kcount(tmp_no_gap, residues = "DNA", k = 12)


SummarizedExperiment::elementMetadata(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$txi)[gene_id %in%
                                                                                                panc_upreg_alu.df]
  as.data.frame() %>% 
  filter(grepl('Alu', gene_id)) -> tx_row_ranges.df


tx_row_ranges.df %>% 
  filter(gene_id %in% panc_upreg_alu.df$ensg,
         grepl('chr', seqnames)) %>% 
  tail(n = 50) -> de_alu_insertion_ranges.df

de_alu_gr <- makeGRangesFromDataFrame(de_alu_insertion_ranges.df, keep.extra.columns = T)

de_alu_seq <- Biostrings::getSeq(BSgenome.Hsapiens.UCSC.hg38, de_alu_gr)

names(de_alu_seq) <- de_alu_gr$tx_name

alu_seq_aln <- DECIPHER::AlignSeqs(de_alu_seq, processors = 5)

alu_seq_distance <- DECIPHER::DistanceMatrix(DECIPHER::RemoveGaps(do.call(c, alu_seq_cons.lst)))

alu_seq_dend <- DECIPHER::IdClusters(alu_seq_distance, type = 'dendrogram')

ggtree::ggtree(alu_seq_dend) + geom_tiplab() + ggplot2::xlim(0, 0.25)

DECIPHER::RemoveGaps(do.call(c, alu_seq_cons.lst))


cons_1 <- DECIPHER::ConsensusSequence(alu_seq_aln)
cons_2 <- DECIPHER::ConsensusSequence(alu_seq_aln)

DECIPHER::BrowseSeqs(DECIPHER::RemoveGaps(do.call(c, alu_seq_cons.lst)))

DECIPHER::ConsensusSequence(DECIPHER::RemoveGaps(do.call(c, alu_seq_cons.lst)), threshold = 0.1)
