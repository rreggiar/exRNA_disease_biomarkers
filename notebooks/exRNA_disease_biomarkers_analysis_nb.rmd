...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                        padj < 0.05, log2FoldChange > 0.75) %>%
  filter(!gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  top_n(wt =-padj, 8) %>% dplyr::pull(gene) -> this_top_10
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.05, log2FoldChange < -1) %>% 
  filter(gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  top_n(wt =-padj, 4) %>% dplyr::pull(gene) -> this_top_10
...
wrap_plots(plt_surv_A.plt,
           guide_area(),
           plt_exp_A.plt + coord_cartesian(clip = 'off') ,
           plt_surv_B.plt,
           plt_exp_B.plt + coord_cartesian(clip = 'off'),
           plt_surv_C.plt,
           plt_exp_C.plt + coord_cartesian(clip = 'off'),
           plt_surv_D.plt,
           plt_exp_D.plt + coord_cartesian(clip = 'off'),
           heights = c(0.9,0.4,0.9,0.9,0.9),
           widths = c(0.75, 1.25),
           guides = 'collect',
