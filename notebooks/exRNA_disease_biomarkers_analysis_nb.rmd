...
flow_chart.plt <- magick::image_read_pdf(path = file.path(figure.dir, 'fig.1', 'plasma_schematic_v4.pdf'))
...
fig.1B.plt <- 
  ...
  annotate(geom = 'text', x = 200, y = -100, label = 'Repeat-aware', size = txt.mm_to_pts(1.25))
  
fig.1C.plt <- 
  ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$deseq$pca$pca_2v3.plt +
  annotate(geom = 'text', x = 200, y = -175, label = 'Repeat-aware', size = txt.mm_to_pts(1.25))

install.packages('lfda')

ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$deseq$vst_counts.df[! rownames(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$deseq$vst_counts.df) %in% 
                         c(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$deseq$age_filter.df$ensg,
                           ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$deseq$sex_filter.df$gene_id), ] %>% 
  t() -> pca_tmp_in 

pca_tmp_in_sd <- apply(pca_tmp_in, 2, sd)
pca_tmp_in_zero_sd <- pca_tmp_in[,pca_tmp_in_sd!=0]

# Y <- ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$quant_meta$condition

# lfda_tmp <- lfda::self(X = pca_tmp_in_zero_sd, Y = Y, metric = 'plain', r = 3, beta = 0.5)

pam_tmp <- cluster::pam(x = pca_tmp_in_zero_sd, k = 3)

merge(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$deseq$pca$pca_out.df,
      pam_tmp$clustering %>% 
        as.data.frame() %>% 
        dplyr::rename('pam_cluster' = '.') %>% 
        rownames_to_column('sample'),
      by = 'sample') %>% 
  ggplot(aes(PC1, PC2, color = condition, shape = as.factor(pam_cluster))) + 
  geom_point(size = 2, alpha = 1) + 
  stat_ellipse(aes(group = as.factor(pam_cluster)), type = 'norm') + 
...
wrap_plots(flowchart_wrap_fig1A.plt,
           ...
           design = fig_1_layout, 
           guides = 'collect',
           heights = c(1,1,0.125,1)) + plot_annotation(tag_levels = 'A') & 
...
tidy_quant_meta.df %>%  
  ...
  ggpubr::stat_compare_means(comparisons = list(c('panc', 'covid'),
                                                c('ctrl', 'covid'), 
                                                c('ctrl', 'panc')), 
                             method = 'wilcox.test', size = 2.5, method.args = list(exact =T)) -> mapping_rate_dist.plt

fig.S1A.plt <- mapping_rate_dist.plt
...
flowchart_wrap_figS1B.plt <- wrap_elements(full = magick::image_ggplot(s1_flow_chart.plt))
```
### Fig_S1_patchwork
```{r}
import::from(.from = 'patchwork', plot_annotation, wrap_plots, guide_area, plot_spacer, wrap_elements)
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  xlab('') + ylab('normalized count fraction') -> fig.2A.plt
```
## Fig_2B
```{r}
...
gencode_layer.plt + 
  ...
  ggrepel::geom_text_repel(data = panc_annotated_rmsk_de %>% 
               ...
               max.overlaps = 1000, box.padding = unit(4.5, 'mm'), segment.colour = 'grey') -> fig.2B.plt
  ...
  ggrepel::geom_text_repel(data = covid_annotated_rmsk_de %>% 
             mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
                    aes(label = ifelse(gene %in% covid_label_list$gene, gene, '')), 
                         show.legend = F, color = 'black', size = txt.mm_to_pts(1), max.overlaps = 1000) + 
...
wrap_plots(fig.2A.plt,
           ...
           design = fig_2_layout, 
           guides = 'collect',
           heights = c(0.75,0.125,0.825,1.225)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
...
wrap_plots(biotype_count_fig_S3) + plot_annotation(tag_levels = 'A') -> patchwork_fig_S3
