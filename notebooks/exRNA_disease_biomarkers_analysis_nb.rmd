...
unique(c(filter(ucsc.rmsk.salmon_quant$analysis_set$covid_ctrl$deseq$de_out, 
                padj < 0.001 & abs(log2FoldChange > 0.25))$ensg,
  filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
         padj < 0.001 & abs(log2FoldChange > 0.25))$ensg)) ->
  de_ensg_names
...
if (! file.exists(file.path(tcga_data_path, 'paad_tcga_gene_count_matrix.tsv')) & 
    ! file.exists(file.path(tcga_data_path, 'paad_gtex_gene_count_matrix.tsv'))) {
  
  tcga_paad_counts <- vroom::vroom(file.path(tcga_data_path, 'paad_tcga_count_matrix.tsv.gz'))
  ...
  vroom::vroom_write(x = tcga_paad_gene_counts, file.path(tcga_data_path, 'paad_tcga_gene_count_matrix.tsv'))
  
  gtex_paad_counts <- vroom::vroom(file.path(tcga_data_path, 'paad_gtex_count_matrix.tsv.gz'))
  ...
  vroom::vroom_write(x = gtex_paad_gene_counts, file.path(tcga_data_path, 'paad_gtex_gene_count_matrix.tsv'))
  
} else {
  message('loading....')
  tx2gene <- read_tsv(file.path(tcga_data_path, 'tcga_gtex_tx2gene.tsv'))
  gtex_paad_gene_counts <- read_tsv(file.path(tcga_data_path, 'paad_gtex_gene_count_matrix.tsv'))
  tcga_paad_gene_counts <- read_tsv(file.path(tcga_data_path, 'paad_tcga_gene_count_matrix.tsv'))
...
tcga_paad_gene_counts %>% 
  ...
  filter(!if_any(where(is.numeric), ~abs(.) > 2.5), gene != 'AluYm1') %>% 
...
hm_count_data %>% 
  ...
  gather(gene, tpm, -sample, -condition) %>% 
...
model_input_data <- hm_count_data[!grepl('-TM$|-NT$', rownames(hm_count_data)), ]
...
str(pROC::roc(response = y_in_test, predictor = testing_pred_matrix[,1]))
