...
fig.S1A_1.plt <- 
  ...
  annotate(geom = 'text', x = -175, y = -100, label = 'Repeat-aware', size = txt.mm_to_pts(1.25)) + coord_flip() 
...
tidy_quant_meta.df %>%  
  ...
  ggforce::facet_col(~var, scales = 'free_y', strip.position = 'bottom') + 
...
fig.S1B.plt <- mapping_rate_dist.plt
```
## Fig_S1C
```{r S1C}
...
fig.S1C.plt <- mapping_rate_compare.plt
...
wrap_plots(fig.S1A_1.plt,
           wrap_elements(full = fig.S1A_2.plt),
           fig.S1B.plt + coord_cartesian(clip = 'off'),
           guide_area(),
