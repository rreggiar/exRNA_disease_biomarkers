...
ucsc.rmsk.salmon_quant$bioIvt_luad_ucsc.rmsk.salmon <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$bioIvt_luad_ucsc.rmsk.salmon, 
                    filter_list = qc_fails)

process.aware.salmon_quant$bioIvt_luad_ucsc.rmsk.salmon <- 
  subset.tximeta.se(process.aware.salmon_quant$bioIvt_luad_process.aware.salmon, 
                    filter_list = qc_fails)
...
luad_v_pitts_meta_for_ip <- read_csv(file.path(meta_data.dir, '2022.03.04_bioIVTLuadAndPitts_meta_data.csv')) %>% 
  filter(patient %in% c(ucsc.rmsk.salmon_quant$pittsburgh_ctrl_ucsc.rmsk.salmon$quant_meta$sample,
                        ucsc.rmsk.salmon_quant$bioIvt_luad_ucsc.rmsk.salmon$quant_meta$sample))

luad_v_pitts_meta_for_ip

luad_v_pitts_meta_for_ip %>% 
  filter(!grepl('luad', patient)) %>% 
  slice_sample(n = 7) %>% 
  ungroup()-> luad_v_pitts_meta_for_filter

luad_v_pitts_meta_for_filter

ucsc.rmsk.salmon_quant$sub_sample_ctrl <-   
  subset.tximeta.se(ucsc.rmsk.salmon_quant$pittsburgh_ctrl_ucsc.rmsk.salmon, 
                    filter_list = luad_v_pitts_meta_for_filter$patient)

luad_v_pitts_meta_for_ip %>% 
  filter(!patient %in% luad_v_pitts_meta_for_filter$patient) -> luad_v_pitts_sample_meta_in

ucsc.rmsk.salmon_quant$analysis_set$luad_ctrl <- 
    build.analysis.set(se_list = ucsc.rmsk.salmon_quant, 
                       se_1 = 'bioIvt_luad_ucsc.rmsk.salmon', 
                       se_2 = 'sub_sample_ctrl',
                       sample_meta_in = luad_v_pitts_meta_for_ip)
...
## Fig_flowchart
```{r}
flow_chart.plt <- magick::image_read_pdf(path = file.path(figure.dir, 'fig.1', 'dkim_plasma_schematic.pdf'))

# flow_chart.plt <- magick::image_read(path = file.path(figure.dir, 'fig.1', 'plasma_schematic.png'))
```
## Fig_1A
```{r 1A}
...
fig.1A_1.plt <- 
  ...
  annotate(geom = 'text', x = -175, y = -100, label = 'TE-aware', size = txt.mm_to_pts(1.25)) + coord_flip() 
...
fig.1B.plt <- mapping_rate_dist.plt
```
## Fig_1C
```{r 1C}
...
fig.1C.plt <- mapping_rate_compare.plt

```

## Fig_1stats
```{r}
SummarizedExperiment::assay(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$txi, 
                            'counts') %>% as.data.frame() -> ucsc_rmsk_counts

ucsc_rmsk_counts <- ucsc_rmsk_counts[, !colnames(ucsc_rmsk_counts) %in% qc_fails]

ucsc_rmsk_counts %>% rownames_to_column('enst') %>%
  merge(reference_meta_data$total_tx_to_gene.df, by = 'enst') -> tmp
...
ucsc_rmsk_biotype_summary %>% 
  ...
  ggpubr::stat_compare_means(method = 'wilcox', comparisons = list(c('covid', 'ctrl'), c('panc', 'ctrl'))) +
  theme(axis.text.x = element_text(angle = 40, hjust = 1),
        legend.position = 'top', legend.direction = 'horizontal') -> biotype_count
...
panc_compare_annotated.df %>% 
  ...
  facet_wrap(~clade, scales = 'free_y', ncol = 2) + 
  theme(axis.text.x = element_text(angle = 40, hjust = 1),
        legend.position = 'top', legend.direction = 'horizontal') -> plt_SN2E.plt
...
wrap_plots(biotype_count) + plot_annotation(tag_levels = 'A') -> biotype_plt

save.manuscript.panel(figure = 's1',
                      figure.dir = figure.dir,
                      name = 'patchwork_full_biotype_tx',
                      width = figure_double_column_width,
                      height = figure_depth,
                      plot.in = biotype_plt)

wrap_plots(plt_SN2E.plt) + plot_annotation(tag_levels = 'A') -> nanopore_biotype_plt


save.manuscript.panel(figure = 's1',
                      figure.dir = figure.dir,
                      name = 'patchwork_full_nanopore_biotype_tx',
                      width = figure_double_column_width,
                      height = figure_depth,
                      plot.in = nanopore_biotype_plt)
...
wrap_plots(fig.1A_1.plt,
           ...
           design = fig_1_layout, 
           guides = 'collect',
           heights = c(0.75,0.75,0.0825,1,1)) + plot_annotation(tag_levels = 'A') & 
...
meta_grob_input_table %>% 
  ...
  gridExtra::tableGrob(theme = gridExtra::ttheme_minimal(base_size = 8,
                       ...
                       rows = NULL) -> fig.S1C.meta_table.grob
...
tidy_total_annotated_rmsk_counts %>% 
  merge(meta_grob_input_table %>% select('sample' = old_name, 'name' = sample), 
        by = 'sample') %>% 
  group_by(name, condition, clade) %>% 
  summarize(clade_sum = sum(count)) %>% 
  mutate(clade_frac = clade_sum/sum(clade_sum),
         condition = factor(condition, levels = c('panc', 'ctrl', 'covid'))) %>% 
  ggplot(aes(name, clade_frac, fill = clade)) + 
  ...
  xlab('') + ylab('normalized count fraction') -> fig.2C.plt
...
flowchart_wrap <- wrap_elements(full = magick::image_ggplot(flow_chart.plt))
wrap_plots(wrap_plots(flowchart_wrap, fig.2C.plt, widths = c(0.25, 0.75)),
           ...
           design = fig_2_layout, 
           guides = 'collect',
           heights = c(1,0.125,0.75,1)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
...
wrap_plots(plt_S2_2A,
           ...
           guides = 'collect',
           heights = c(1,0.05,1)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
        legend.title = element_text(vjust = 0.5),
...
panc.6.1.1_nanopore_hex_to.plt %>% 
  ...
  xlab('avg. observed SINE fragment size (bp)') + ylab('expected SINE insert size (bp)') +
...
ctrl.6.1.5_nanopore_hex_to.plt %>%
  ...
  xlab('avg. observed fragment size (bp)') + ylab('expected SINE insert size (bp)') +
  # ggforce::facet_col(~sample) +
  ggtitle('ctrl 6') + 
...
panc_compare_annotated.df %>% 
  ...
  xlab('') + 
  scale_fill_manual(values = viridisLite::plasma(20)[c(1,8,10)]) +
  facet_wrap(~clade, scales = 'free_y', ncol = 4) -> plt_SN2E.plt
...
ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out %>% 
  filter(padj < 0.01) %>%
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene') %>% 
  mutate(family = ifelse(ensg %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg,
                         'gencode_lncRNA', family),
         family = ifelse(is.na(family), 'gencode_coding', family),
         clade = ifelse(grepl('gencode', family), family, clade),
         clade = ifelse(clade %in% c('gencode_coding',
                                     ...
                                     'Simple_repeat'), clade, 'other'),
         type = ifelse(grepl('gencode',family), 'GENCODE', 'TE'),
         clade = factor(clade, levels = te_aware_annotation_levels)) -> feature_de.df
...
feature_de.df %>% 
  ...
  # as.matrix() %>%
  # as.data.frame() %>%
  # rownames_to_column('gene') %>% select(gene, 'beta' = V1), by = 'gene') %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'te_sig_de.csv'))
...
rowRanges(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$gxi) %>% 
  as.data.frame() %>% 
  filter(group_name %in% reference_meta_data$gencode_tx_to_gene.df$ensg) -> range_tmp

feature_de.df %>% 
  merge(beta %>%
  as.matrix() %>%
  as.data.frame() %>%
  rownames_to_column('ensg') %>% select(ensg, 'beta' = V1), by = 'ensg') %>% 
  merge(range_tmp, by.x = 'ensg', by.y = 'group_name', all.x = T) %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'te_aware_sig_de.csv'))
...
wrap_plots(wrap_elements(full = gt_binom_roc.plt + inset_element(gt_binomial_cm.tbl, right = 0.5, 
                                         bottom = 0.2, left = 0.6, top = 0.5, ignore_tag = T)),
           gt_feature_de.plt + annotate(geom = 'text', label = 'TE-aware', x = -1, y = 7),
           wrap_elements(full = binom_roc.plt + inset_element(binomial_cm.tbl, right = 0.5, 
                                         bottom = 0.2, left = 0.6, top = 0.5, ignore_tag = T)),
           feature_de.plt + annotate(geom = 'text', label = 'TE-alone', x = -1, y = 3.5),
           guide_area(),
           plot_spacer(),
...
save.manuscript.panel(figure = 's4',
                      figure.dir = figure.dir,
                      name = 'patchwork_full_fig_model',
                      width = figure_double_column_width,
                      height = figure_depth,
...
### IP tables
```{r}
build_ip_tables <- function(analysis_set,
                            annotated_de,
                            zero_cutoff = 5){
  
  import::here(SummarizedExperiment, rowRanges, assay)
  
  rowRanges(analysis_set$gxi) %>% 
    as.data.frame() %>% 
    filter(group_name %in% filter(annotated_de, 
                                  type == 'GENCODE',
                                  padj < 0.001,
                                  log2FoldChange > 1)$ensg) %>% 
    merge(annotated_de %>% select(gene, ensg, padj, baseMean, log2FoldChange), 
          by.x = 'group_name', by.y = 'ensg') %>% 
    select(gene, chr = seqnames, start, end, width, baseMean, padj, log2FoldChange) %>% 
    mutate(reference = 'hg38_gencode_v39') -> gencode_ip_table.tmp

  rowRanges(analysis_set$txi) %>% 
    as.data.frame() %>% 
    remove_rownames() %>% 
    filter(gene_id %in% filter(annotated_de, 
                                  type == 'TE',
                                  padj < 0.001,
                                  log2FoldChange > 1)$ensg) -> rmsk_enrich_1
  
  assay(analysis_set$txi, 'counts') %>% 
    as.data.frame() %>% 
    select(contains('panc')) %>% 
    rownames_to_column('tx') %>% 
    filter(tx %in% rmsk_enrich_1$tx_name) -> insert_count
  
  insert_count %>% 
    filter(rowSums(across(where(is.numeric)) == 0) <= zero_cutoff) -> keep_inserts
  
  rmsk_enrich_1 %>% 
    filter(tx_name %in% keep_inserts$tx) %>% 
    select(gene = gene_id, chr = seqnames, start, end, width, strand, tx_name) %>% 
    mutate(chr = as.character(chr),
           gene = as.character(gene)) %>% 
    merge(annotated_de %>% 
          select(gene, baseMean, padj, log2FoldChange), by = 'gene') %>% 
    select(gene, tx_name, chr, start, end, width, baseMean, padj, log2FoldChange) %>% 
    mutate(reference = 'hg38_ucscGB_rmsk')-> rmsk_ip_table.tmp
  
  return(list('gencode_ip_table' = gencode_ip_table.tmp,
              'rmsk_ip_table' = rmsk_ip_table.tmp))
}


unique(panc_ip$rmsk_ip_table$gene)

lapply(unique(panc_ip$rmsk_ip_table$gene), function(te){
  
  panc_ip$rmsk_ip_table %>% filter(gene == te) -> x
  
  write_csv(x, file = file.path(output_data.dir, 'ip_data',
                                paste0('panc_', te, '.csv')))
  
})

# imap(covid_ip, function(x, name){
#   
#   write_csv(x, file = file.path(output_data.dir,
#                                 paste0('covid_', name, '.csv')))
#   
# })
...
panc_annotated_rmsk_de %>%
  ...
  scale_color_manual(values = te_color_pal) + 
  xlim(-5,5) +
  ggrepel::geom_text_repel(aes(label = ifelse(log2FoldChange > 1.5, gene, '')),
                             show.legend = F, color = 'black', max.overlaps = 10)

luad_annotated_rmsk_de %>%
  filter(clade %in% c('gencode_coding', 'gencode_lncRNA')) %>% 
  ggplot(aes(log2FoldChange, -log10(padj), color = clade)) + 
  geom_point(size = 2, alpha = 0.6, show.legend = FALSE) + 
  scale_color_manual(values = te_color_pal) + 
  xlim(-7.5,7.5) +
  ggrepel::geom_text_repel(aes(label = ifelse(log2FoldChange > 1.5, gene, '')),
                             show.legend = F, color = 'black', max.overlaps = 50)
...
gencode_layer.plt + 
  geom_point(data = filter(luad_annotated_rmsk_de, clade != 'gencode_coding') %>% 
               mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
             aes(log2FoldChange, -log10(padj), color = clade), size = 2, alpha = 0.8) +
  scale_color_manual(values = te_color_pal) + 
  xlim(-7.5,7.5) +
  ggrepel::geom_text_repel(data = luad_annotated_rmsk_de %>% 
               mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
                      aes(label = ifelse(log2FoldChange > 2, gene, '')), 
                           show.legend = F, color = 'black', max.overlaps = 10) -> luad_ip.plt

luad_ip.plt

de_compare_list <- list('panc' = ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out,
                        'luad' = ucsc.rmsk.salmon_quant$analysis_set$luad_ctrl$deseq$de_out)
...
de_upset.df %>% 
  ...
  geom_bar(fill = 'white', color = 'black') + 
  ggupset::scale_x_upset() +
  geom_text(stat='count', aes(label=after_stat(count)), nudge_y = -2.25) + 
  xlab('') + ylab('Enriched gene expression overlap')
```
## Final IP tables
```{r}
ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out %>% 
...
feature_de.df %>% 
  select(gene, baseMean, padj, log2FoldChange, clade, family) %>% 
  mutate(reference = 'hg38', annotation = 'Repeat Masker') %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'panc_te_sig_de.csv'))

ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out %>% 
  filter(!ensg %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene, !ensg %in% chrM.filter.list)%>%
  filter(padj < 0.01) %>% 
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene', all.x = T) %>% 
  mutate(family = ifelse(ensg %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg,
                         'gencode_lncRNA', family),
...
feature_de.df %>% 
  merge(range_tmp, by.x = 'ensg', by.y = 'group_name') %>% 
  select(gene, 'chr' = seqnames, start, end, strand, 
         baseMean, padj, 'biotype' = family, log2FoldChange) %>% 
  mutate(reference = 'hg38', annotation = 'GENCODEv39') %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'panc_gencode_sig_de.csv'))
...
feature_de.df %>% 
  select(gene, baseMean, padj, log2FoldChange, clade, family) %>% 
  mutate(reference = 'hg38', annotation = 'Repeat Masker') %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'covid_te_sig_de.csv'))
...
feature_de.df %>% 
  ...
  select(gene, 'chr' = seqnames, start, end, strand, 
         baseMean, padj, 'biotype' = family, log2FoldChange) %>% 
  mutate(reference = 'hg38', annotation = 'GENCODEv39') %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'covid_gencode_sig_de.csv'))
...
feature_de.df %>% 
  ...
  select(gene, 'chr' = seqnames, start, end, strand, 
         baseMean, padj, 'biotype' = family, log2FoldChange) %>% 
  mutate(reference = 'hg38', annotation = 'GENCODEv39') %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'panc_MT_sig_de.csv'))
...
feature_de.df %>% 
  ...
  select(gene, 'chr' = seqnames, start, end, strand, 
         baseMean, padj, 'biotype' = family, log2FoldChange) %>% 
  mutate(reference = 'hg38', annotation = 'GENCODEv39') %>% 
  write_csv(file.path(output_data.dir, 'ip_data', 'covid_MT_sig_de.csv'))
...
tcga_padd_stratified_counts %>% 
  ...
  mutate(gene.set = 'panc_exRNA_signal') %>%
  group_by(gene.set) %>%
...
surv.fit <- survfit(as.formula(surv.obj ~ panc_exRNA_signal), 
...
paste0('panc gencode up \n (n genes = ',filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                        padj < 0.05, log2FoldChange > 0.75) %>% 
  filter(!gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>% nrow(), ')') -> km_title
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
  ...
  top_n(wt =-padj, 10) %>% 
  arrange(padj) %>% 
  select(gene, baseMean, log2FoldChange, padj) -> panc_up_km_table

panc_up_km_table %>% 
  gridExtra::tableGrob(theme = gridExtra::ttheme_minimal(base_size = 8,
                                                         core = list(padding=unit(c(1, 2.5), "mm"))), 
                       rows = NULL) -> plt_surv_A.tbl
...
surv.plt$plot + ggtitle('panc gencode down') -> plt_surv_B.plt
```
### DN lncRNA
```{r}
reference_meta_data$gencode_lncRNA_gene_names.df %>% 
  select(-biotype) %>% 
  merge(reference_meta_data$total_tx_to_gene.df, by ='ensg') -> lncRNA_tx_to_gene
...
tcga_padd_stratified_counts %>% 
  ...
  filter(gene %in% lncRNA_tx_to_gene$gene) %>%
  select(sample, count) %>% 
  group_by(sample) %>%
  summarize(avg.count = mean(count)) %>%
  mutate(gene.set = 'panc_exRNA_signal') %>%
...
surv.plt$plot + ggtitle('panc lncRNA down') -> plt_surv_C.plt
```
### DN TEs
```{r}
tcga_padd_stratified_counts <-
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
  ...
  top_n(wt =-padj, 10) %>% 
  arrange(padj) %>% 
  select(gene, baseMean, log2FoldChange, padj) -> panc_dn_TE_km_table

panc_dn_TE_km_table %>% 
  gridExtra::tableGrob(theme = gridExtra::ttheme_minimal(base_size = 8,
                                                         core = list(padding=unit(c(1, 2.5), "mm"))), 
                       rows = NULL) -> plt_surv_D.tbl
```
### UP TEs
```{r}
tcga_padd_stratified_counts <-
...
tcga_padd_stratified_counts %>% 
  ...
  gather(gene, count, -sample) %>% 
  filter(gene %in% filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.05, log2FoldChange > 0.5)$gene) %>% 
  filter(gene %in% reference_meta_data$ucsc_rmsk_insert_info.df$gene) %>%
  select(sample, count) %>% 
  group_by(sample) %>%
  summarize(avg.count = mean(count)) %>%
...
surv.plt$plot + ggtitle('panc TE up') -> plt_surv_E.plt
```
### OX PHOS
```{r}
tcga_padd_stratified_counts <-
...
paste0('Ox. Phos \n (n genes = ',filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.05, log2FoldChange > 0.75) %>%
  filter(gene %in% msig.df$HALLMARK_OXIDATIVE_PHOSPHORYLATION) %>% nrow(), ')') -> km_title

surv.plt$plot + ggtitle(km_title) -> plt_surv_F.plt
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
  ...
  top_n(wt =-padj, 10) %>% 
  arrange(padj) %>% 
  select(gene, baseMean, log2FoldChange, padj) -> panc_OX_km_table

panc_OX_km_table %>% 
  gridExtra::tableGrob(theme = gridExtra::ttheme_minimal(base_size = 8,
                                                         core = list(padding=unit(c(1, 2.5), "mm"))), 
                       rows = NULL) -> plt_surv_F.tbl
```
### MYC
```{r}
tcga_padd_stratified_counts <-
...
paste0('MYC Targets \n (n genes = ',filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                          padj < 0.05, log2FoldChange > 0.75) %>%
  filter(gene %in% msig.df$HALLMARK_MYC_TARGETS_V1) %>% nrow(), ')') -> km_title

surv.plt$plot + ggtitle(km_title) -> plt_surv_G.plt
...
filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
  ...
  top_n(wt =-padj, 10) %>% 
  arrange(padj) %>% 
  select(gene, baseMean, log2FoldChange, padj) -> panc_MYC_km_table

panc_MYC_km_table %>% 
  gridExtra::tableGrob(theme = gridExtra::ttheme_minimal(base_size = 8,
                                                         core = list(padding=unit(c(1, 2.5), "mm"))), 
                       rows = NULL) -> plt_surv_G.tbl
```
### plt_surv_patchwork
```{r}
import::from(.from = 'patchwork', plot_annotation, wrap_plots, guide_area, plot_spacer, wrap_elements)
...
wrap_plots(plt_surv_F.plt,
           guide_area(),
           patchwork::wrap_elements(full = plt_surv_F.tbl, ignore_tag = F),
           plt_surv_G.plt,
           patchwork::wrap_elements(full = plt_surv_G.tbl, ignore_tag = F),
           plt_surv_A.plt,
           patchwork::wrap_elements(full = plt_surv_A.tbl, ignore_tag = F),
           plt_surv_D.plt,
           patchwork::wrap_elements(full = plt_surv_D.tbl, ignore_tag = F),
           heights = c(1,0.095,1,1,1,1,1),
           guides = 'collect',
           design = fig_surv_layout) +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag.position = c(0.01, 0.99), 
