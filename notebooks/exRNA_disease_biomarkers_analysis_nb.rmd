...
ucsc_rmsk_counts %>% 
  ...
  merge(reference_meta_data$total_tx_to_gene.df %>% select(), by = 'gene') -> ucsc_rmsk_counts
...
ucsc_rmsk_biotype_summary %>% 
  filter(!grepl('\\?', biotype)) %>% 
  ggplot(aes(condition, count/1000, fill = condition)) + 
  geom_col() + 
  ylab('1000s of detected transcripts') + 
  xlab('') + 
  scale_y_continuous(labels = scales::number_format(accuracy = 0.01)) +
  scale_fill_manual(values = identity_color_pal) + 
  facet_wrap(~biotype, scales = 'free_y', ncol = 3) + 
  theme(axis.text.x = element_text(angle = 40, hjust = 1), text = element_text(size = 7),
        legend.position = c(0.98, 0.01), legend.direction = 'horizontal') -> biotype_count
...
panc.6.1.1_nanopore_hex_to.plt %>% 
  ...
  ggtitle('nanopore exp. vs obs. size -- panc 6') + 
...
wrap_plots(plt_NE.plt + xlab('') + ylab(''),
           plt_SNA_1.plt + xlab(''),
           plt_SNA_3.plt + ylab(''),
...
wrap_plots(plt_NC.plt,
           wrap_elements(full = panel.SNA.patchwork),
           guide_area(),
           plt_ND_2.plt,
           # wrap_elements(full = plt_ND_2.plt),
           design = fig_N_layout, 
           guides = 'collect',
           heights = c(1,1,1,0.035,1)) +
