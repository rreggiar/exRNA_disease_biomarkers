...
wrap_plots(fig.S1A.plt,
           ...
           fig.S1C.plt,
           guide_area(),
           fig.S1D+ coord_cartesian(clip = 'off'),
           fig.S1E,
           plot_spacer(),
           design = fig_S1_layout, 
           guides = 'collect',
           heights = c(0.8,0.8,0.2,1,1)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
        legend.title = element_text(vjust = 1.25),
...
total_annotated_rmsk_counts %>% 
  ...
  ggplot() +
  ggrastr::rasterise(geom_point(aes(x = .panel_x, y = .panel_y), size = txt.mm_to_pts(0.2), alpha = 0.4)) +
  theme(axis.text.x = element_blank()) +
  ggforce::facet_matrix(vars(panc.1.2.3:panc.6.1.1)) -> panc_paired_scatter
...
wrap_plots(panc_paired_scatter,
...
calc_entropy(
  ...
  theme(axis.text.x = element_text(angle = 40, hjust = 1),
        strip.text = element_text(size = txt.mm_to_pts(1.5))) -> fig.S3.F
```
...
wrap_plots(ma_plt_list[[1]] + ggtitle('liver cancer'),
           ma_plt_list[[2]] + ggtitle('lung cancer'),
           ma_plt_list[[3]] + ggtitle('esophagus cancer'),
           ma_plt_list[[4]] + ggtitle('colorectal cancer'),
           ma_plt_list[[5]] + ggtitle('stomach cancer'),
           guide_area(),
           fig.S3.F,
           # wrap_elements(full = fig.3F.plt + ylim(0,95) + ggtitle('upregulated TEs')),
           # wrap_elements(full = fig.3E.plt + ylim(0,95) + ggtitle('downregulated TEs')),
...
str(final_alpha$liver$entropy)



lapply(final_alpha, function(cancer) {
...
imap(alpha_order, function(index, cancer) {
  ...
  best_model <-
    final_alpha[[cancer]][[feature_set]][[best_alpha]]

  gencode_model <-
    final_alpha[[cancer]][['gencode']][[gencode_alpha]]
...
model_comparison$colorectal$best_model$model$glmnet.fit[,which.min(model_comparison$colorectal$best_model$model$glmnet.fit$lambda)]

model_comparison$colorectal$best_model$model$glmnet.fit
...
lapply(model_comparison, function(model) {
  lapply(model, function(comparison) {
    ...
    coef_tmp <-
      cbind(coef(comparison$model, s = comparison$model$lambda.min))

    beta <- coef_tmp[apply(coef_tmp != 0, 1, any),]

    colnames(beta) <-
      names(coef(comparison$model, s = comparison$model$lambda.min))
...
length(training_performance$colorectal$best_model$beta)
...
lapply(names(count_out_list), function(cancer) {
  training_performance[[cancer]]$best_model$roc$data %>%
    ...
    xlab('1 - specificity') +
    ggtitle(cancer) +
    theme(legend.position = c(0.9, 0.55)) -> roc.plt
  ...
  training_performance[[cancer]]$best_model$cutoffs %>%
    ...
    geom_point() +
    geom_errorbar() +
    ylim(0, 1) + xlab('Repeat') +
    ylab('sensitivity @ 90% specificity') +
  ...
  testing_performance[[cancer]]$best_model$cutoffs %>%
    ...
    geom_errorbar() +
    ylim(0, 1) + xlab('Repeat') +
    ylab('sensitivity @ training thresh.') +
...
wrap_plots(wrap_elements(full = plot_list[[1]]$roc),
           plot_list[[1]]$train,
           plot_list[[1]]$test,
  ...
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
        legend.title = element_text(vjust = 0.5),
        legend.justification = c(0.5, 0.5)) -> fig.4.patchwork
