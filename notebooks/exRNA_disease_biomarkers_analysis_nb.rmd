...
ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out %>% 
  ...
  geom_point() + 
  scale_color_manual(values = te_alone_color_pal[names(te_alone_color_pal) %in% 
                                                   c('LINE', 'SINE', 'LTR', 'tRNA', 'DNA')]) + 
...
wrap_plots(wrap_elements(full = binom_roc.plt + inset_element(binomial_cm.tbl, right = 0.5, 
           ...
           feature_de.plt + theme(legend.position = c(0.99,0.99)),
           plot_spacer(),
           # guides = 'collect',
           design = fig_surv_layout) +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag.position = c(0.01, 0.99)) -> fig.model.patchwork
