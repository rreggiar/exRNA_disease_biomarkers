...
original_roster_names %>% 
  filter(name %in% qc_fails) %>% 
  pull(name) ->qc_fail_names
...
internal_quant$original_data <-
  subset.tximeta.se(
    internal_quant$ucsc_rmsk_quant,
    filter_list = c(
      qc_fail_names,
      normal_samples,
      unknown_samples,
...
internal_quant$origCovid_normal <-
  subset.tximeta.se(
    internal_quant$ucsc_rmsk_quant,
    filter_list = c(
      qc_fail_names,
...
internal_quant$origPanc_normal <-
  subset.tximeta.se(
    internal_quant$ucsc_rmsk_quant,
    filter_list = c(
      qc_fail_names,
...
elife_select_list <-
  filter(
    external_quant$ucsc_rmsk_quant$quant_meta,
    !sample %in% elife_merge_meta_for_filt$name
  ) %>% 
  pull(sample)
...
```{r}
merge(
  gut_quant$ucsc_rmsk_quant$quant_meta %>%
      mutate(name = sample, sample = str_split_fixed(sample, '_G', 2)[, 1]),
    gut_meta_data,
    by = 'sample'
...
# quant meta rename
```{r}
internal_quant$original_data$quant_meta <-
  merge(
    internal_quant$original_data$quant_meta %>%
      mutate(tmp = sample, sample = str_split_fixed(sample, '_S', 2)[, 1]),
    original_roster_names,
    by = 'sample'
  ) %>% mutate(sample = tmp) %>% select(-tmp)

internal_quant$origCovid_normal$quant_meta <-
  merge(
    internal_quant$origCovid_normal$quant_meta %>%
      mutate(tmp = sample, sample = str_split_fixed(sample, '_S', 2)[, 1]),
    origCovid_normal_meta_data,
    by = 'sample'
  ) %>% mutate(name = sample, sample = tmp) %>% select(-tmp)
...
gut_quant$ucsc_rmsk_quant$deseq <- 
  run.de.seq.individual(input_se = gut_quant$ucsc_rmsk_quant,
                        type = 'gxi',
                        base_level = 'healthy',
                        top_level = 'PDAC',
                        ref_type = 'ucsc.rmsk',
                        dds_formula = 
                          as.formula('~diagnosis'))
...
rowRanges(gut_quant$ucsc_rmsk_quant$gxi) %>% 
  as.data.frame() %>% 
...
## Fig_2A
```{r}
gut_quant$ucsc_rmsk_quant$deseq$norm_counts.df %>% 
...
total_annotated_rmsk_counts %>% 
  ...
  # mutate(sample = ifelse(is.na(name), sample, name)) %>% 
  # select(-name) %>% 
  merge(gut_quant$ucsc_rmsk_quant$deseq$scaled_quant_meta_for_de.df %>%
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  mutate(clade_frac = clade_sum/sum(clade_sum),
         condition = factor(condition, levels = c('healthy', 'PDAC'))) %>% 
  select(-clade_sum) %>% 
  # mutate(clade_frac = log(clade_frac)) %>% 
  pivot_wider(names_from = clade, values_from = clade_frac) %>% 
  merge(gut_quant$ucsc_rmsk_quant$preRad_preRes$quant_meta, by = 'sample') -> chm_in
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  merge(gut_quant$ucsc_rmsk_quant$quant_meta, by = 'sample') %>% 
...
## Fig_2B
```{r}
gut_quant$ucsc_rmsk_quant$deseq$de_out %>% 
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene', all.x = T) %>% 
...
gencode_layer.plt + 
  ...
  ggrepel::geom_text_repel(data = panc_annotated_rmsk_de %>% 
               ...
               max.overlaps = 20, box.padding = 0.3, segment.colour = 'grey') +
