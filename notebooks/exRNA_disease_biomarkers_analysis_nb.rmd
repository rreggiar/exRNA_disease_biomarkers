...
internal_rmsk_quant$panc_normal$deseq$norm_counts.df %>%
  ...
  write_csv(file.path(output_data.dir, 'panc_normal_normalized_counts_20230703.csv'))
...
internal_rmsk_quant$panc_normal$quant_meta %>%
  ...
  ggpubr::stat_compare_means(comparisons = list(c('Repeat-naive', 'Repeat-aware')), 
                           ...
                           method.args = list(exact = TRUE, paired = T)) -> fig.1D.plt
...
internal_rmsk_quant$panc_normal$deseq$norm_counts.df %>%
  ...
  ggpubr::stat_compare_means(comparisons = list(c('panc', 'normal')), 
                         method = 'wilcox', size = txt.mm_to_pts(1.2), 
                         method.args = list(exact = TRUE, paired = F)) -> fig.1E.plt
```
### Fig_1_patchwork
```{r Fig1}
...
internal_rmsk_quant$panc_normal$deseq$norm_counts.df %>%
  ...
  mutate(
    ...
    clade = ifelse(
      clade %in% c(
        te_alone_annotation_levels[1:5]
      ),
      clade,
      'other'
    ),
    type = ifelse(grepl('GENCODE', family), 'GENCODE', 'TE'),
    clade = factor(clade, levels = te_alone_annotation_levels[1:5])
  ) %>%
...
tidy_total_annotated_rmsk_counts %>%
  ...
  ggplot(aes(sample, clade_frac, fill = clade, color = diagnosis)) +
  geom_col(color = 'white', show.legend = T) +
  scale_fill_manual(values = te_alone_color_pal[1:5], name = 'biotype/clade') +
  ggforce::facet_row(~ diagnosis + stage, scales = 'free_x', space = 'free') +
  scale_y_continuous(expand = c(0, 0)) +
  xlab('N = {normal: 22, panc: 10}') + ylab('normalized count fraction') +
  theme(axis.text.x = element_blank(),
...
panc_nanopore_read_length.df %>% 
  ...
  theme(legend.position = c(0.55,0.99), legend.direction = 'horizontal', plot.margin = margin(r = 3, unit = 'mm')) -> plt_S5A.plt
```
### Fig_S5B
```{r}
import::from(.from = 'patchwork', plot_annotation, wrap_plots, guide_area, plot_spacer)
...
wrap_plots(plt_NE.plt + xlab(''),
           plt_SNA_3.plt,
           design = panel_SNA_layout) -> panel.S5B.patchwork
```
### Fig_S5C
```{r}
...
panc.6.1.1_nanopore_hex_to.plt %>%
  ...
  theme(legend.position = c(0.25, 0.99)) -> plt_S5C.plt
```
### Fig_S5_patchwork
```{r}
import::from(.from = 'patchwork', plot_annotation, wrap_plots, guide_area, plot_spacer, wrap_elements)
...
wrap_plots(plt_S5A.plt,
           wrap_elements(full = panel.S5B.patchwork, clip = F),
           guide_area(),
           plt_S5C.plt + coord_cartesian(clip = 'off'),
...
panc_compare_annotated.df %>% 
  ...
  scale_color_manual(values = te_color_pal) + 
  scale_fill_manual(values = te_color_pal)-> plt_S6A.plt
```
### Fig_S6B
```{r}
...
panc_compare_annotated.df %>% 
  ...
  annotate(geom = 'text', x = 6, y = 16, fontface = 'italic',
         ...
         size = rel(3.75), alpha = 0.8) +
  scale_color_manual(values = te_color_pal) + 
  scale_fill_manual(values = te_color_pal) -> plt_S6B.plt
```
### Fig_S6_patchwork
```{r}
import::from(.from = 'patchwork', plot_annotation, wrap_plots, guide_area, plot_spacer, wrap_elements)
...
wrap_plots(plt_S6A.plt + ggtitle('panc 7 nanopore v illumina'),
           guide_area(),
           plt_S6B.plt + ggtitle('panc 6 nanopore v illumina'),
           guides = 'collect',
