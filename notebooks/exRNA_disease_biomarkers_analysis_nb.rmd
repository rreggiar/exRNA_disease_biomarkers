...
ucsc.rmsk.salmon_quant$analysis_set$luad_ctrl$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$analysis_set$luad_ctrl,
                        type = 'gxi',
                        base_level = 'luad',
                        top_level = 'ctrl',
                        ref_type = 'ucsc.rmsk',
...
panc.7.1.1_nanopore_tpm <- 
  read_tsv(file.path(output_data.dir, 'nanopore', 'gencode_39_rmsk', 'panc.7.1.1_gene_abundance.tsv'))

panc.6.1.1_nanopore_tpm <- 
  read_tsv(file.path(output_data.dir, 'nanopore', 'gencode_39_rmsk', 'panc.6.1.1_gene_abundance.tsv'))

panc.7.1.1_read_len <- read_csv(file.path(output_data.dir, 'nanopore', 'panc.7.1.1_biotype_read_lengths.csv')) %>% 
  separate(sample, sep = '[.]', into = c(NA,NA,'biotype',NA)) %>% 
  mutate(biotype = sub('_extraction', '', biotype))

panc.6.1.1_read_len <- read_csv(file.path(output_data.dir, 'nanopore', 'panc.6.1.1_biotype_read_lengths.csv')) %>% 
  separate(sample, sep = '[.]', into = c(NA,NA,'biotype',NA)) %>% 
  mutate(biotype = sub('_extraction', '', biotype))
...
len_compare.df %>% 
  ...
  mutate(biolabel = factor(biolabel, levels = te_aware_annotation_levels)) %>% 
  # mutate(gene = str_split_fixed(tx, pattern = '_', n = 4)[,1])
  ggplot(aes(read_length, len, color = sample)) + 
  geom_point(alpha = 0.2) + 
  xlab('avg. observed length') + ylab('expected length') +
  ggforce::facet_col(~biolabel, scales = 'free') +
  scale_color_manual(values = RColorBrewer::brewer.pal(3, 'Dark2'))

illumina_read_len <- list.files(file.path(output_data.dir, 'nanopore', 'illumina_read_lengths'), full.names = T)
names(illumina_read_len) <- gsub(basename(illumina_read_len), pattern = '.csv', replacement = '')
...
illumina_read_len.df %>% 
  filter(biotype %in% nanopore_biotype_labels$biotype) %>% 
  group_by(biotype) %>%
  merge(nanopore_biotype_labels, by = 'biotype') %>% 
  mutate(biolabel = factor(biolabel, levels = te_aware_annotation_levels)) %>% 
  ggplot(aes(abs(read_length))) + 
  geom_histogram(aes(y=stat(density*width)), bins = 100) + 
  ggforce::facet_col(~biolabel, scales = 'free') 
...
len_compare.df %>% 
  filter(biotype.x == 'SINE') %>% 
  select(tx, sequence = reference_name) -> alu_len.df
...
alu_len.lst <- as.list(alu_len.df$sequence)
names(alu_len.lst) <- alu_len.df$tx

alu_read.string <- Biostrings::AAStringSet(unlist(alu_len.lst))

alu_aln <- DECIPHER::AlignSeqs(alu_read.string, processors = 5)
...
panc.7.1.1_nanopore_tpm %<>% 
  group_by(`Gene ID`) %>% 
  summarize(nanopore_tpm = sum(TPM))

as.data.frame(as.matrix(assay(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$gxi, 
                              'counts')[, 'panc.7.1.1', drop = F])) %>% 
  rownames_to_column('Gene ID') %>% 
  merge(panc.7.1.1_nanopore_tpm, by = 'Gene ID') %>% 
  dplyr::rename('ensg' = 'Gene ID') %>% 
  mutate(illumina_tpm = panc.7.1.1*1e6/(sum(panc.7.1.1))) -> panc.7.1.1_seq_compare.df

panc.7.1.1_seq_compare.df %<>%
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by.x = 'ensg', by.y = 'gene', all.x = T) %>% 
  mutate(family = ifelse(ensg %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg,
                         'gencode_lncRNA', family),
         family = ifelse(is.na(family), 'gencode_coding', family),
         clade = ifelse(grepl('gencode', family), family, clade),
...
panc.7.1.1_seq_compare.df %>%
  ggplot(aes(nanopore_tpm, illumina_tpm, color = clade)) + 
  geom_point() +
  scale_color_manual(values = te_color_pal) + 
  ggforce::facet_col(~clade, scales = 'free') +
  geom_smooth(method = 'lm', show.legend = F) + 
  ggrepel::geom_text_repel(aes(label = ifelse(ensg %in% filter(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl$deseq$de_out, 
                                                               padj < 0.01 & log2FoldChange > 0)$ensg, ensg, '')), 
                           show.legend = F)
...
as.data.frame(as.matrix(assay(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$gxi, 'counts')[, 'panc.6.1.1', drop = F])) %>% 
  rownames_to_column('Gene ID') %>% 
...
panc.7.1.1_seq_compare.df %>% 
  ...
  filter(nanopore_tpm < 50000 & illumina_tpm < 50000) %>% 
  ggplot(aes(nanopore_tpm, illumina_tpm, color = clade)) + 
  geom_point() + 
  scale_color_manual(values = te_color_pal) 
...
total_compare.df <- 
  merge(panc.7.1.1_seq_compare.df,
        panc.6.1.1_seq_compare.df, by = 'ensg')

total_compare.df %>% 
  ggplot(aes(illumina_tpm.x, illumina_tpm.y, color = clade.x)) + 
  geom_hline(yintercept = seq(0,80000, 20000), alpha = 0.2) +
  geom_vline(xintercept = seq(0,80000, 20000), alpha = 0.2) +
  geom_point() + 
  scale_color_manual(values = te_color_pal) +
  ylab('panc.6.1.1') + xlab('panc.7.1.1') + 
  ggtitle('illumina tpm comparison')
...
panc_ip <- build_ip_tables(ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl,
                           panc_annotated_rmsk_de,
                           zero_cutoff = 5)

ucsc.rmsk.salmon_quant$analysis_set$luad_ctrl$deseq$de_out %>% 
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene', all.x = T) %>% 
  mutate(family = ifelse(ensg %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg,
                         'gencode_lncRNA', family),
         family = ifelse(is.na(family), 'gencode_coding', family),
         ...
         clade = factor(clade, levels = te_aware_annotation_levels)) %>% 
  filter(!ensg %in% chrM.filter.list,
         !grepl('^MT', gene)) -> luad_annotated_rmsk_de

luad_ip <- build_ip_tables(ucsc.rmsk.salmon_quant$analysis_set$luad_ctrl,
                            luad_annotated_rmsk_de,
                            zero_cutoff = 5)

table(luad_ip$rmsk_ip_table$gene %in% panc_ip$rmsk_ip_table$gene)

imap(panc_ip, function(x, name){
  
  write_csv(x, file = file.path(output_data.dir, 'ip_data',
                                paste0('panc_', name, '.csv')))
...
filter(panc_annotated_rmsk_de, clade != 'gencode_coding') %>% 
             mutate(clade = factor(clade, levels = te_aware_annotation_levels)) %>% 
  ggplot(aes(log2FoldChange, -log10(padj), color = clade, label = gene)) + 
  geom_point(size = 2, alpha = 0.8) +
  scale_color_manual(values = te_color_pal) + 
  ggrepel::geom_text_repel(aes(label = ifelse(grepl('Alu', gene), gene, '')),
                               show.legend = F, color = 'black', max.overlaps = 1000)
