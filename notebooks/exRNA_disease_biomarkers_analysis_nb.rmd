...
## data_path_obj
```{r}
figure.dir <- here::here('proj/manuscript/figures')
input_data.dir <- here::here('proj/data/input_data')
output_data.dir <- here::here('proj/data/output_data')
qc_data.dir <- here::here('proj/data/output_data/rna_qc')
meta_data.dir <- here::here('proj/data/meta_data')
reference.dir <- here::here('proj/reference')
r_code.dir <- here::here('proj/R')
```
...
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant, 
                    filter_list = c('unknown1_S49.salmon.out',
...
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta <- 
  merge(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>% 
        mutate(sample = str_split_fixed(sample, '_',2)[,1]), 
    discovery_meta_data, by = 'sample')
```
...
disc_identity_color_pal <- setNames(identity_color_pal, c('normal', 'panc', 'esoph'))
# disc_pca <-
...
total_annotated_rmsk_counts %>% 
  ...
  merge(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq$scaled_quant_meta_for_de.df %>%
          select(sample, condition, stage, treatment_status_resection_treatment_status), 
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  group_by(sample, condition, stage, treatment_status_resection_treatment_status, clade) %>% 
  ...
  ggforce::facet_row(~treatment_status_resection_treatment_status+condition+stage, scales = 'free_x', space = 'free') +
...
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>% 
  ...
  geom_jitter() + 
  # ggforce::facet_row(~stage, space = 'free') + 
  ylim(0,100)
```
## Fig_2B
```{r}
