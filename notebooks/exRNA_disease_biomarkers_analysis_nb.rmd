...
dataset_identity_color_pal <- setNames(RColorBrewer::brewer.pal(4, 'Dark2'),
                                   unique(united_analysis_set$quant_meta %>% pull(dataset)))

diagnosis_identity_color_pal <- setNames(RColorBrewer::brewer.pal(8, 'Dark2'),
                                   unique(united_analysis_set$quant_meta %>% pull(diagnosis)))


input_de <- united_analysis_set$deseq

input_de$vst_counts.df[!rownames(input_de$vst_counts.df) %in%
                           c(united_analysis_set$deseq$sex_filter.df$group_name), ] %>%
    t() -> pca_tmp_in
...
pca.out %>% 
  ...
  ggplot(aes(PC1, PC2,
             fill = dataset)) +
  geom_point(size = txt.mm_to_pts(0.8), shape = 21, alpha = 0.8, aes(color = after_scale(alpha(color, 1)))) +
  ...
  scale_fill_manual(values = dataset_identity_color_pal) + 
  xlim(-100,250) + 
  ylim(-50,100)
