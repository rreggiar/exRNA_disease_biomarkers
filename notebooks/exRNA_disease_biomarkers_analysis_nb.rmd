...
## discovery
covid_samples <- ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>% filter(grepl('^HMN', sample)) %>% pull(sample)
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant <-
  subset.tximeta.se(
    ...
    filter_list = c(
      qc_fails,
      # covid_samples,
      'unknown1_S49.salmon.out',
      'unknown2_S50.salmon.out',
      'unknown3_S51.salmon.out',
      'unknown4_S52.salmon.out',
      'unknown5_S53.salmon.out',
      'e14_S32.salmon.out'
      # 'e11_S8.salmon.out',
      # 'e12_S16.salmon.out',
      # 'e13_S24.salmon.out',
...
## new nanopore
```{r}
nanopore_read_len <- read_csv(file.path(output_data.dir, 
                                        'resub_nanopore_read_length_counts.csv'),
                              col_names = c('sample', 'len')
                              )
...
nanopore_read_len %>% 
  mutate(diagnosis = 
           case_when(
             ...
             grepl('P', sample) ~ 'panc',
             grepl('E', sample) ~ 'esoph',
             grepl('^[[:digit:]]', sample) ~ 'ctrl'
           )
         ) %>% 
  mutate(rep = str_split_fixed(sample, pattern = '[.]', n = 2)[,1]) -> label_nanopore_read_len

label_nanopore_read_len %>% 
  filter(len >= 10, len <= 1500) %>% 
  ggplot(aes(len, color = rep)) + 
...
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$ucsc_rmsk_quant,
                        ...
                        ref_type = 'ucsc.rmsk',
                        dds_formula = 
                          as.formula('~sample_sku_patient_age_at_collection+input_vol+condition'))
...
disc_identity_color_pal <- setNames(RColorBrewer::brewer.pal(4, 'Dark2'), 
                                    c('normal', 'panc', 'esoph', 'covid'))
# disc_pca <-
