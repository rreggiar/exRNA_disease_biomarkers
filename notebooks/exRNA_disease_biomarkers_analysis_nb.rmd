...
lapply(elife_rmsk_quant, function(analysis_set){
...
elife_rmsk_quant$rmsk$deseq$norm_counts.df %>%
  ...
  rownames_to_column('ensg') %>%
  filter(
    ensg %in% c(reference_meta_data$ucsc_rmsk_insert_info.df$gene),
    ensg %in%        filt_elife_de_list$ensg
  ) %>%
  filter(across(where(is.numeric), ~ any(.x != 0))) %>%
  rowwise() %>%
  mutate(sum = sum(c_across(where(is.numeric)))) %>%
...
elife_rmsk_quant$rmsk$pca <- 
  run.pca(input_de = elife_rmsk_quant$rmsk$deseq, te_only = TRUE)
