...
illumina_read_len <- list.files(file.path(output_data.dir, 'nanopore', 'illumina_read_len'), full.names = T)[1:30]
names(illumina_read_len) <- gsub(basename(illumina_read_len), pattern = '.csv', replacement = '')[1:30]
...
name_conversion$out_name <- c('panc.1.2.3', 'panc.2.2.7', 'panc.4.3.9', 
                              ...
                              'panc.10.1.2', 'covid.400.0.56', 'covid.381.0.80',
...
illumina_read_len.df %>% 
  filter(biotype %in% illumina_biotype_labels$biotype) -> illumina_biotype_to.plt #%>% 
  group_by(biotype) %>%
  merge(illumina_biotype_labels, by = 'biotype') %>%
  mutate(biolabel = factor(biolabel, levels = te_aware_annotation_levels)) -> illumina_biotype_to.plt

illumina_biotype_to.plt %>% 
  filter(!sample %in% qc_fails) %>% 
  group_by(biotype, condition) %>% 
  ggplot(aes(abs(read_length), fill = condition)) + 
  geom_histogram(aes(y=stat(density*width)), bins = 50, color = 'white',
                 position = position_dodge()) + 
  facet_wrap(~biolabel+condition, scales = 'free_y', nrow = 4)
...
tmp_mean_read_len.df %>%
  # filter(abs(read_length) > 75) %>% 
  group_by(biotype, condition) %>%
  filter(n() > 8, ! biotype %in% c('polymorphic_pseudogene', 'TR_C_gene', 'TR_V_gene', 'IG_V_gene', 'IG_C_gene',
  ...
  facet_wrap(~biotype, scales = 'free', ncol = 3, strip.position = 'left') +
  ...
  xlab('') + ylab('median insert length') -> plt_NA.plt
```
## Fig_NB
```{r}
tmp_mean_read_len.df %>% 
...
panc_nanopore_read_length.df %>% 
  ...
  annotate(geom = 'text', x = 225, y = 0.1, label = 'SINE') +
  scale_fill_manual(values = RColorBrewer::brewer.pal(3, 'Dark2'), ) +
  ylab('proportion') + xlab('nanopore fragment length') +
...
illumina_biotype_to.plt %>% 
  ...
  summarize(read_length = mean(abs(read_length))) -> tmp
...
ctrl_hex_to.plt %>% 
  ...
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm') +
  scale_y_continuous(breaks = c(50,100,150,200,300,400)) + 
  scale_x_continuous(breaks = c(median(ctrl_hex_to.plt$read_length), 50,100,150,200,300,400)) + 
  theme(legend.position = c(0.95,0.125), legend.direction = 'horizontal', legend.key.width = unit(5, 'mm'))
...
panc_hex_to.plt %>% 
  ...
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm') +
  scale_y_continuous(breaks = c(50, 100,150,200,300,400)) + 
  scale_x_continuous(breaks = c(median(panc_hex_to.plt$read_length),50,100,150,200,300,400)) + 
  theme(legend.position = c(0.95,0.125), legend.direction = 'horizontal', legend.key.width = unit(5, 'mm'))


illumina_biotype_to.plt %>% 
  filter(biotype == 'SINE',
         sample_name %in% c('panc.7.1.1', 'panc.9.1.1', 'ctrl.6.1.5')) %>% 
...
ctrl_hex_to.plt %>% 
  ...
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'ctrl.6.1.5') +
  geom_vline(xintercept = median(ctrl_hex_to.plt$read_length), color = 'grey', linetype = 'dashed') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', breaks = c(0,1)) +
  theme(legend.position = c(0.99,0.2), legend.direction = 'horizontal', legend.key.width = unit(1, 'mm')) +
  guides(fill = guide_colorbar(title.position = 'left', title.vjust = 1)) -> plt_ND_1.plt
...
panc.7.1.1_hex_to.plt %>% 
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'panc.7.1.1') +
  geom_vline(xintercept = median(panc.7.1.1_hex_to.plt$read_length), color = 'grey', linetype = 'dashed') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', breaks = c(0,1), guide = 'none') -> plt_ND_2.plt


tmp %>%
  filter(sample_name == 'panc.9.1.1') -> panc.6.1.1_hex_to.plt

panc.6.1.1_hex_to.plt %>% 
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'REPLACE') +
  geom_vline(xintercept = median(panc.6.1.1_hex_to.plt$read_length), color = 'grey', linetype = 'dashed') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', breaks = c(0,1), guide = 'none') -> plt_ND_3.plt

p4 <- ggplot(data.frame(l = plt_ND_3.plt$labels$x, x = 1, y = 1)) +
...
wrap_plots(plt_ND_1.plt + ylab('') + xlab(''),
           plt_ND_2.plt + xlab('') + ylab(''),
           plt_ND_3.plt + xlab('') + ylab(''),
           p4,
           # guides = 'collect',
           heights = c(1,1,1, 0.1),
...
ctrl.6.1.5_nanopore_hex_to.plt %>% 
  ...
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) + 
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 275, y = 175, label = 'ctrl.6.1.5') +
  geom_vline(xintercept = median(ctrl.6.1.5_nanopore_hex_to.plt$read_length), color = 'grey', linetype = 'dashed') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  # ggforce::facet_col(~sample) +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', guide = 'none') +
  scale_y_continuous(breaks = c(100,200,300,400), limits = c(75,400)) +
  scale_x_continuous(breaks = c(100,200,300,400), limits = c(75,400)) -> plt_NE_1.plt
...
panc.6.1.1_nanopore_hex_to.plt %>% 
  ...
  annotate(geom = 'text', x = 275, y = 175, label = 'panc.6.1.1') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  # ggforce::facet_col(~sample) +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', breaks = c(0,1)) +
  scale_y_continuous(breaks = c(100,200,300,400), limits = c(75,400)) +
  scale_x_continuous(breaks = c(100,200,300,400), limits = c(75,400)) + 
...
panc.7.1.1_nanopore_hex_to.plt %>% 
  ...
  annotate(geom = 'text', x = 275, y = 175, label = 'panc.7.1.1') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  # ggforce::facet_col(~sample) +
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', guide = 'none') +
  scale_y_continuous(breaks = c(100,200,300,400), limits = c(75,400)) +
  scale_x_continuous(breaks = c(100,200,300,400), limits = c(75,400)) -> plt_NE_3.plt

p4 <- ggplot(data.frame(l = plt_NE_3.plt$labels$x, x = 1, y = 1)) +
      geom_text(aes(x, y, label = l), angle = 0) + 
      theme_void() +
      coord_cartesian(clip = "off")
...
wrap_plots(plt_NE_1.plt + ylab('') + xlab(''),
           plt_NE_2.plt + xlab('') + ylab(''),
           plt_NE_3.plt + xlab('') + ylab(''),
           # p4,
           heights = c(1,1,1),
           design = panel_NE_layout) -> panel.NE.patchwork
```
...
wrap_plots(plt_NA.plt,
           ...
           plt_NB.plt,
           plt_NC.plt,
           wrap_elements(full = panel.NE.patchwork),
           wrap_elements(full = panel.ND.patchwork),
           design = fig_N_layout, 
           guides = 'collect',
...
illumina_biotype_to.plt %>% 
  filter(condition == 'ctrl',
         biotype == 'SINE',
         abs(read_length) < 50) -> tmp

illumina_biotype_to.plt %>% 
  filter(condition == 'ctrl',
         biotype == 'SINE',
         abs(read_length) > 200) -> tmp_2
