...
}) %>% bind_rows() %>% distinct() -> united_metadata_df
...
internal_quant <- load.tximeta.object.list('full_resub',
                                           output_data.dir = output_data.dir)


internal_quant <-
  parse.tximeta.quant.metadata(internal_quant)

external_quant <-
  load.tximeta.object.list('other_extData_ucsc_rmsk',
                           output_data.dir = output_data.dir)


external_quant <-
  parse.tximeta.quant.metadata(external_quant)

gut_quant <- load.tximeta.object.list('final_extData_ucsc_rmsk',
                                      output_data.dir = output_data.dir)


gut_quant <- parse.tximeta.quant.metadata(gut_quant)
...
merge(
  internal_quant$ucsc_rmsk_quant$quant_meta %>%
...
orig_covid_samples <-
  internal_quant$ucsc_rmsk_quant$quant_meta %>% 
  filter(grepl('^HMN', sample)) %>% 
  pull(sample)

orig_panc_samples <-
  internal_quant$ucsc_rmsk_quant$quant_meta %>%
  filter(grepl('panc|BioIVT|bioIVT', sample)) %>%
  pull(sample)

unknown_samples <-
  internal_quant$ucsc_rmsk_quant$quant_meta %>% filter(grepl('^unknown', sample)) %>% pull(sample)

pitts_samples <-
  internal_quant$ucsc_rmsk_quant$quant_meta %>% filter(grepl('^PH', sample)) %>% pull(sample)

esoph_samples <-
  internal_quant$ucsc_rmsk_quant$quant_meta %>% filter(grepl('^e', sample)) %>% pull(sample)

normal_samples <-
  internal_quant$ucsc_rmsk_quant$quant_meta %>% filter(grepl('^n', sample)) %>% pull(sample)
...
ucsc.rmsk.salmon_quant$original_data <-
  subset.tximeta.se(
...
merge(
  ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>%
      mutate(name = sample, sample = str_split_fixed(sample, '_G', 2)[, 1]),
    npj_meta_data %>% filter(diagnosis %in% c('non-cancer', 'liver cirrhosis')),
    by = 'sample'
...
npj_select_list <-
  filter(
    ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta,
    !sample %in% npj_merge_meta_for_filt$name
  ) %>% 
  pull(sample)

ucsc.rmsk.salmon_quant$ucsc_rmsk_quant <-
  subset.tximeta.se(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant,
...
# 
# ucsc.rmsk.salmon_quant$original_data$quant_meta <-
#   ucsc.rmsk.salmon_quant$original_data$quant_meta %>%
#     mutate(tmp = sample,
#            sample = str_split_fixed(sample, '_S', 2)[, 1]) %>%
...
#     mutate(sample = str_split_fixed(tmp, '_S', 2)[, 1])
# 
# ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta <-
#   merge(
#     ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>%
...
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$preRad_preRes$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$preRad_preRes,
                        type = 'gxi',
                        base_level = 'non-cancer',
                        top_level = 'liver cancer',
                        ref_type = 'ucsc.rmsk',
                        dds_formula = 
                          as.formula('~sample_sku_patient_age_at_collection+sex+diagnosis'))
...
ucsc.rmsk.salmon_quant$original_data$deseq <-
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$original_data,
                        type = 'gxi',
                        base_level = 'normal',
                        top_level = 'panc',
                        ref_type = 'ucsc.rmsk',
                        dds_formula = as.formula('~sample_sku_patient_age_at_collection+condition'))
# 
# 
...
disc_identity_color_pal <- setNames(RColorBrewer::brewer.pal(5, 'Dark2'),
                                   unique(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>% pull(diagnosis)))
...
pca.out <- as.data.frame(non_zero_pca$x) %>% 
  ...
  # mutate(name = str_split_fixed(sample, '_G', 2)[, 1]) %>% 
  # merge(original_roster_names, by = 'sample', all.x = T) %>% 
  # mutate(sample = ifelse(is.na(name), sample, name)) %>% 
  # select(-name) %>% 
  merge(input_de$scaled_quant_meta_for_de.df %>% rownames_to_column('name'), by = 'name')
...
total_annotated_rmsk_counts %>% 
  ...
  # mutate(sample = ifelse(is.na(name), sample, name)) %>% 
  # select(-name) %>% 
  merge(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq$scaled_quant_meta_for_de.df %>%
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  select(-clade_sum) %>% 
  # mutate(clade_frac = log(clade_frac)) %>% 
  pivot_wider(names_from = clade, values_from = clade_frac) %>% 
  merge(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$preRad_preRes$quant_meta, by = 'sample') -> chm_in

hm_count_data_filt <- chm_in %>% 
  select(sample, GENCODE_lncRNA:other) %>% 
  column_to_rownames('sample') %>% 
  as.matrix()

meta_data <- chm_in %>% select(-GENCODE_coding:-other)
rownames(meta_data) <- meta_data$sample
meta_data <-
  meta_data[match(rownames(meta_data), rownames(hm_count_data_filt)),]

meta_data <- meta_data %>%
  mutate(
    sample_sku_primary_diagnosis = str_replace_all(sample_sku_primary_diagnosis, 'Pancreatic Cancer, ', '')
  ) %>%
  pull(sample_sku_primary_diagnosis)

panc_color <- setNames(RColorBrewer::brewer.pal(name = 'Dark2', n = 4), c('panc', 'normal', 'esoph', 'covid'))

left_hma <-
  ComplexHeatmap::rowAnnotation(
    df = meta_data %>% select(sample_sku_primary_diagnosis),
    col = list(condition = panc_color),
...
# mkp_de_sig %>%
#   filter(gene %in% colnames(hm_count_data_filt)) %>%
#   column_to_rownames('gene') -> toil_de
# 
# toil_de[order(match(rownames(toil_de), colnames(hm_count_data_filt))), , drop =
#           F] -> toil_de
# 
# toil_de_list <- setNames(toil_de$log2FC, rownames(toil_de))
# 
# ComplexHeatmap::columnAnnotation(
#   `MkP Log2FC` = ComplexHeatmap::anno_barplot(
#     x = toil_de_list,
#     bar_width = 1,
#     which = 'row',
#     gp = grid::gpar(col = "black", fill = 'purple'),
#     border = FALSE,
...
ComplexHeatmap::Heatmap(
  as.matrix(hm_count_data_filt),
  show_row_names = F,
  show_column_names = T,
  # col = viridis::plasma(n = 100),
  # heatmap_width = figure_double_column_width * 1.125,
  ...
  # top_annotation = log2fc_ha,
  column_names_gp = grid::gpar(fontsize = 12),
  name = 'Percent'
) -> hsc_count.chm

patchwork::wrap_elements(full = grid::grid.grabExpr(ComplexHeatmap::draw(hsc_count.chm),
                                                    # width = figure_double_column_width* 1.125,
                                                    merge_legend = T)) -> chm_plot

chm_plot
# ggsave(
#   chm_plot,
#   width = figure_double_column_width * 1.125,
#   height = panel_height_1 * 1.65,
#   filename = file.path(output_data.dir, 'mkp_de_heatmap.pdf'),
#   units = 'mm'
# )
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  mutate(clade_frac = clade_sum/sum(clade_sum)) %>% 
         # condition = factor(condition, levels = c('panc', 'normal', 'esoph', 'covid'))) %>% 
  merge(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta, by = 'sample') %>% View()
  ...
  ggforce::facet_col(~condition, scales = 'free_x', space = 'free') +
...
## Fig_2B
```{r}
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$preRad_preRes$deseq$de_out %>% 
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene', all.x = T) %>% 
...
gencode_layer.plt + 
  ...
  ggrepel::geom_text_repel(data = panc_annotated_rmsk_de %>% 
               mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
                      aes(label = ifelse(padj < 0.05, gene, '')), 
...
x_in <- t(ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq$norm_counts.df)

y_in <- ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq$scaled_quant_meta_for_de.df %>% 
  rownames_to_column('sample') %>% 
  select(sample, condition) %>% 
...
perf_list <- sens_at_spec_ROC(binom_roc_out, 1)
#
predictions <- ROCR::prediction(labels = y, predictions = y_hat_matrix, label.ordering = c(0,1))
# 
