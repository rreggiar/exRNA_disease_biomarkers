...
united_quant_meta %>% 
  merge(united_metadata_df, by = 'sample') -> united_analysis_set$quant_meta
...
pca.out %>% 
  ...
  ggplot(aes(PC1, PC2,
             color = diagnosis)) +
  geom_point(size = txt.mm_to_pts(0.8), shape = 21, aes(fill = after_scale(alpha(color, 0.3)))) +
...
umap_out %>% 
  as.data.frame() %>% 
  rownames_to_column('sample') %>% 
  merge(united_analysis_set$deseq$scaled_quant_meta_for_de.df %>% rownames_to_column('sample'), by = 'sample') %>% 
  ggplot(aes(V1, V2,
             color = dataset)) +
  # scale_shape_manual(values = c(21)) +
  geom_point(size = txt.mm_to_pts(0.8), aes(fill = after_scale(alpha(color, 0.3))), shape = 21)
