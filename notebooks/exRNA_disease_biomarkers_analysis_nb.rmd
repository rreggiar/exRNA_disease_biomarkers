...
unsplice_counts_pca.df %>% 
  ...
  unsplice.df <- assay(split.gxi, "unspliced") %>% as.data.frame() %>% 
    rownames_to_column("gene") %>% gather(sample, count, 
    -gene) %>% group_by(sample) %>% summarize(unsplice_count = sum(count))
  splice.df <- assay(split.gxi, "spliced") %>% as.data.frame() %>% 
    rownames_to_column("gene") %>% gather(sample, count, 
    -gene) %>% group_by(sample) %>% summarize(splice_count = sum(count))
  merge(unsplice.df, splice.df, by = "sample") %>% mutate(unsplice_rate = unsplice_count/(unsplice_count + 
    splice_count))
...
unsplice_filt.df %>% 
  ...
  merge(sample_meta_data$`2022.01.14_combined_meta_data.csv`, by.x = 'sample', by.y = 'patient') %>% 
  ggplot(aes(log1p(unsplice_count))) +
  geom_histogram() + 
  facet_wrap(~condition+clade)
```
### Fig_4_patchwork
```{r}
import::from(.from = 'patchwork', plot_annotation, wrap_plots, guide_area, plot_spacer, wrap_elements)
