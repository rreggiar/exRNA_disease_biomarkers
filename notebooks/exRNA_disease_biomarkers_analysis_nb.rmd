...
wrap_plots(fig.S1A.plt,
           ...
           guides = 'collect',
           heights = c(0.8,0.8,0.2,1.4)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
        legend.title = element_text(vjust = 1.25),
...
calc_entropy(
  ...
  theme(axis.text.x = element_text(angle = 40, hjust = 1),
        strip.text = element_text(size = txt.mm_to_pts(1.5)))
```
...
imap(count_out_list, function(norm_counts.df, name) {
  ...
  imap(feature_set_list, function(feature_set, feature_set_name) {
    ...
    binom_elife_x_in <-
      binom_model_input_data[, colSums(binom_model_input_data != 0) > 0]
