...
fig.1A_1.plt <- 
  ...
  labs(tag = 'B') + 
  annotate(geom = 'text', x = -100, y = -100, label = 'TE-aware', size = txt.mm_to_pts(0.8))
...
wrap_plots(fig.1A_1.plt + coord_cartesian(clip = 'off'),
           ...
           design = fig_1_layout, 
           guides = 'collect',
           heights = c(1.35,0.0825,0.65,0.65,0.45)) & 
...
panc_label_list <- rbind(panc_label_list, panc_label_list_dn)
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  xlab('sample') + ylab('library count fraction') -> fig.2C.plt
...
ctrl.6.1.5_nanopore_hex_to.plt %>%
  ...
  scale_x_continuous(breaks = c(50,100,200,300,400), limits = c(0,400)) -> plt_SNA_1.plt
...
panc.6.1.1_nanopore_hex_to.plt %>% 
  ...
  annotate(geom = 'text', x = 275, y = 150, label = 'panc 6') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  # ggforce::facet_col(~sample) +
...
panc.7.1.1_nanopore_hex_to.plt %>% 
  ...
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', guide = 'none') +
  scale_y_continuous(breaks = c(100,200,300,400), limits = c(75,450)) +
  scale_x_continuous(breaks = c(100,200,300,400), limits = c(75,450)) -> plt_SNA_3.plt
...
ctrl_hex_to.plt %>%
  ggplot(aes(read_length, len, fill = scales::rescale((..count../(sum(..count..) * 1e6))))) +
  geom_hex(bins = 120) +
  annotate(geom = 'text', x = 250, y = 150, label = 'ctrl 6') +
  # geom_vline(xintercept = median(ctrl_hex_to.plt$read_length), color = 'grey', linetype = 'dashed') +
  xlab('avg. observed fragment length') + ylab('expected insertion length') +
  scale_y_continuous(breaks = c(50,100,200,300,400), limits = c(0,400)) +
