...
internal_quant$original_data$deseq <- 
  run.de.seq.individual(input_se = internal_quant$original_data,
                        type = 'gxi',
                        base_level = 'normal',
                        top_level = 'panc',
                        ref_type = 'ucsc.rmsk',
...
calc_entropy(
  internal_quant$original_data$deseq$norm_counts.df,
  original_discovery_meta_data,
  grouping_level = 'type'
) %>%
  # ggplot(aes(diagnosis, entropy)) +
...
#                                     c('normal', 'panc'))
input_de <- internal_quant$origPanc_normal$deseq

disc_identity_color_pal <- setNames(RColorBrewer::brewer.pal(2, 'Dark2'),
                                   unique(input_de$quant_meta %>% pull(diagnosis)))
...
input_de$vst_counts.df[!rownames(input_de$vst_counts.df) %in%
                           c(input_de$sex_filter.df$group_name), ] %>%
    t() -> pca_tmp_in
...
pca.out %>% 
  ...
  #                       exclude = NULL)) %>% 
  ggplot(aes(PC1, PC2,
             color = condition, shape = dataset)) +
  geom_point(size = txt.mm_to_pts(0.8), aes(fill = after_scale(alpha(color, 0.2)))) +
