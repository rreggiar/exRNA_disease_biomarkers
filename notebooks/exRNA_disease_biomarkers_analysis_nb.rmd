...
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta <-
  ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>%
    ...
    select(-name) %>% 
    merge(combined_meta_data %>% filter(diagnosis != 'covid'), by = 'sample')
...
disc_identity_color_pal <- setNames(RColorBrewer::brewer.pal(3, 'Dark2'), 
                                    c('normal', 'panc', 'esoph'))
# disc_pca <-
...
## 1vsall
```{r}
PAAD_1vAll <- read_csv(file.path(output_data.dir, 'PAAD_vs_all.results'), skip = 7) %>% filter(log2FoldChange > 1)
Pancreas_1vAll <- read_csv(file.path(output_data.dir, 'Pancreas_vs_all.results'), skip = 7) %>% filter(log2FoldChange > 1)

PAAD_1vAll %>% 
  filter(! name %in% Pancreas_1vAll$name) -> filt_PAAD_1vAll
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  mutate(clade_frac = clade_sum/sum(clade_sum),
         condition = factor(condition, levels = c('panc', 'normal', 'esoph'))) %>%
...
ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$quant_meta %>% 
  ggplot(aes(diagnosis, salmon_frag_length_mean)) + 
  geom_boxplot(fill = NA) + 
  geom_jitter() +
  ggforce::facet_row(~dataset)
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  filter(clade == 'SINE') %>% 
  ggplot(aes(salmon_percent_mapped, median_sine_abundance, color = condition)) + 
  geom_point()
  # geom_boxplot(fill = NA) + 
  # geom_jitter() +
