...
gene_set <- filter(internal_rmsk_quant$panc_normal$deseq$de_out,
                   padj < 0.01,
                   log2FoldChange > 0.75)$gene
...
adj_surv_final <- merge_luad_surv_final %>%
  ...
  mutate(
    signature = factor(signature, levels = c('low', 'high')),
    age = scale(age)[, 1]
  )
...
cox_fit <-
  coxph(Surv(time = OS.time, event = OS) ~ signature,
        data = tmp_surv,
        x = T)
...
tmp_pval <-
  adjusted_curve_test(adjsurv,
                      from = 0,
                      to = max(adjsurv$adjsurv$time),)$p_value
adjsurv$adjsurv %>%
  group_by(group) %>%
  ggplot(aes(
    time,
  ...
  pammtools::geom_stepribbon(aes(ymin = ci_lower, ymax = ci_upper),
                             alpha = 0.085,
                             linetype = 0) +
  scale_color_manual('panc UP signature',
...
stage_list <- list('stage 1', 'stage 2/3')
names(stage_list) <- c('stage 1', 'stage 2/3')

imap(stage_list, function(input_stage, name) {
  set.seed(1323)
  adj_surv_final %>%
    filter(stage == input_stage) -> tmp_surv
  
  cox_fit <-
    coxph(
      Surv(time = OS.time, event = OS) ~ age + gender + signature,
      data = tmp_surv,
      x = T
    )
  ...
  adjsurv$adjsurv$stage <- input_stage
...
lapply(final_surv_list, function(set) {
  set$`stage 2/3`$pval
})


final_surv_list$`HALLMARK UNFOLDED PROTEIN RESPONSE`$`stage 2/3`$surv_df$adjsurv %>%
  group_by(group) %>%
  ggplot(aes(
    time,
    y = surv,
  ...
  pammtools::geom_stepribbon(aes(ymin = ci_lower, ymax = ci_upper),
                             alpha = 0.085,
                             linetype = 0) +
  scale_color_manual('UNFOLDED PROTEIN',
                     values = c(viridis::viridis(3)[1], viridis::viridis(3)[2])) +
  scale_fill_manual(values = c(viridis::viridis(3)[1], viridis::viridis(3)[2]),
                    guide = 'none') +
  annotate(
    ...
    label = paste0(
      'pval = ',
      round(
        final_surv_list$`HALLMARK UNFOLDED PROTEIN RESPONSE`$`stage 2/3`$pval,
        4
      )
    ),
    size = txt.mm_to_pts(0.8)
