...
y_in <- hm_meta_data %>% mutate(condition = ifelse(condition == 'PAAD', 1, 0))

x_in_train <- x_in[trainSplitIndex, ,drop = F]
y_in_train <- y_in[trainSplitIndex, ,drop = F] %>% pull(condition)

x_in_test <- x_in[-trainSplitIndex, ,drop = F]
y_in_test <- y_in[-trainSplitIndex, ,drop = F] %>% pull(condition)

set.seed(123)

tcga_binomial_model <- 
  cv.glmnet(x = x_in_train,
            y = y_in_train,
            family = 'binomial',
            type.measure = 'deviance',
            nfolds = 20, 
...
training_pred_matrix <- predict(tcga_binomial_model, s = 'lambda.min', newx = x_in_train, type = 'response')

pROC::roc(response = y_in_train, predictor = training_pred_matrix[,1])$auc


testing_pred_matrix <- predict(tcga_binomial_model, s = 'lambda.min', newx = x_in_test, type = 'response')

pROC::roc(response = y_in_test, predictor = testing_pred_matrix[,1])
