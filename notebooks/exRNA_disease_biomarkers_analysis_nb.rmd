...
united_analysis_set$deseq <- 
  run.de.seq.individual(input_se = united_analysis_set,
                        type = 'gxi',
                        base_level = 'healthy',
                        top_level = 'PDAC',
                        ref_type = 'ucsc.rmsk',
...
diagnosis_identity_color_pal <- setNames(RColorBrewer::brewer.pal(2, 'Dark2'),
                                   unique(input_de$quant_meta %>% pull(diagnosis)))
...
pca.out <- as.data.frame(non_zero_pca$x) %>% 
  ...
  # select(-name) %>% 
  merge(united_analysis_set$quant_meta, by = 'sample')
...
uwot::umap(pca_tmp_in_zero_sd, n_neighbors = 20, pca = 100) -> umap_out
...
umap_out %>% 
  ...
  ggplot(aes(V1, V2,
             fill = diagnosis)) +
  scale_fill_manual(values = diagnosis_identity_color_pal) + 
  geom_point(size = txt.mm_to_pts(0.8), shape = 21, alpha = 0.75, aes(color = after_scale(alpha(color, 1))))
...
## Fig_2A
```{r}
internal_quant$origCovid$deseq$norm_counts.df %>% 
  rownames_to_column('gene') %>% 
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene', all.x = T) %>% 
  mutate(family = ifelse(gene %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg,
                         'GENCODE_lncRNA', family),
...
total_annotated_rmsk_counts %>% 
  ...
  # mutate(sample = ifelse(is.na(name), sample, name)) %>% 
  # select(-name) %>% 
  merge(internal_quant$origCovid$quant_meta, 
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  merge(internal_quant$origCovid_normal$quant_meta, by = 'sample') %>% 
  ggplot(aes(reorder(sample, input_vol), clade_frac, fill = clade, color = diagnosis)) +
...
## Fig_2B
```{r}
internal_quant$origCovid_normal$deseq$de_out %>% 
  merge(reference_meta_data$ucsc_rmsk_insert_info.df, by = 'gene', all.x = T) %>% 
