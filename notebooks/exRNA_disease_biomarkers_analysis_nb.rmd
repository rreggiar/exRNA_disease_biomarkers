...
ucsc.rmsk.salmon_quant <- load.tximeta.object.list('full_resub',
                                                   output_data.dir = output_data.dir)
...
y_in <- ucsc.rmsk.salmon_quant$ucsc_rmsk_quant$deseq$scaled_quant_meta_for_de.df %>% 
  rownames_to_column('sample') %>% 
  select(sample, condition) %>% 
  mutate(condition = ifelse(condition == 'normal', 0, 1)) %>% 
  column_to_rownames('sample')
...
classifications <- ifelse(binomial_model_response_prob > perf_list$p.thresh, 0, 1)

# y_hat_predictions <- predict(object = binom_model, s = 'lambda.min', 
#                              newx = x_in, newy = y_in, type = 'response')
...
predictions <- ROCR::prediction(labels = y, predictions = y_hat_matrix, label.ordering = c(1,0))
# 
