...
tmp_read_len <- read_csv(file.path(output_data.dir, 'nanopore', 'bioIVT_panc_1_2_3.biotype_read_lengths.csv')) %>% 
  separate(sample, sep = '[.]', into = c(NA,NA,'biotype',NA)) %>% 
  mutate(biotype = sub('_extraction', '', biotype))
...
tmp_read_len %>% 
  ...
  ggforce::facet_col(~biotype, scales = 'free') 
...
as.data.frame(as.matrix(assay(ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$gxi, 'counts')[, 'panc.7.1.1', drop = F])) %>% 
