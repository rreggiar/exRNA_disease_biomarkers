...
panc_annotated_rmsk_de %>%
  ...
  ggrepel::geom_text_repel(aes(label = ifelse(gene %in% panc_label_list$gene, gene, '')), 
                           show.legend = F, color = 'black', max.overlaps = 100) -> gencode_layer.plt
...
gencode_layer.plt + 
  geom_point(data = filter(panc_annotated_rmsk_de, clade != 'gencode_coding') %>% 
             ...
             aes(log2FoldChange, -log10(padj), color = clade), size = 2, alpha = 0.8, show.legend = F) +
  ...
  ggrepel::geom_text_repel(data = covid_annotated_rmsk_de %>% 
             mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
                    aes(label = ifelse(gene %in% covid_label_list$gene, gene, '')), 
                         show.legend = F, color = 'black', max.overlaps = 1000) + 
  labs(tag = 'B') + xlim(-7.5,7.5) -> fig.2B.plt
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  xlab('sample') + ylab('norm. count fraction') + 
  labs(tag = 'C') -> fig.2C.plt
...
salmon_quant$analysis_set$ctrl_panc_covid$deseq$norm_counts.df %>% 
  ...
  merge(salmon_quant$analysis_set$ctrl_panc_covid$deseq$scaled_quant_meta_for_de.df, by = 'sample') %>% 
  merge(reference_meta_data$gencode_lncRNA_gene_names.df, by = 'ensg', all.x = T) -> salmon_gencode_biotype 
...
wrap_plots(patchwork::wrap_elements(fig.S2A.fgsea_table.grob, ignore_tag = F),
           ...
           guides = 'collect',
           heights = c(0.35,1,1,1)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
        legend.title = element_text(vjust = 0.5),
