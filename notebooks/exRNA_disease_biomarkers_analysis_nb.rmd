...
import::from(.from = 'patchwork', plot_annotation, wrap_plots, guide_area)
...
salmon_quant$analysis_set$ctrl_panc_covid$deseq$pca$pca_total.plt + theme(legend.position = 'none')
...
tidy_quant_meta.df %>%  
  ...
  geom_point(position = position_jitter(width = 0.2), aes(shape = sex), size = rel(2), alpha = 0.8) +
...
mapping_rate_dist.plt + labs(tag = 'B') + theme(plot.tag.position = c(0.01, 0.99))
...
mapping_rate_compare.df %>% 
  ...
  geom_boxplot(fill = NA, outlier.colour = NA, show.legend = F) + 
  geom_point(aes(shape = sex), size = rel(2), alpha = 0.8) +
...
wrap_plots(salmon_quant$analysis_set$ctrl_panc_covid$deseq$pca$pca_1v2.plt + 
             theme(legend.position = 'none') + labs(tag = 'A'),
           salmon_quant$analysis_set$ctrl_panc_covid$deseq$pca$pca_2v3.plt + theme(legend.position = 'none'),
           mapping_rate_dist.plt + labs(tag = 'B'),
           guide_area(),
           mapping_rate_compare.plt + labs(tag = 'C'),
           design = fig_1_layout, guides = 'collect',
           heights = c(1,1,0.125,1)) & theme(plot.tag.position = c(0.01, 0.99), 
