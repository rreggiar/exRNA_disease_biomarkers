...
ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid <- 
    build.analysis.set(se_list = ucsc.rmsk.salmon_quant, 
                       se_1 = 'bioIvt_covid_ucsc.rmsk.salmon', 
                       analysis_set_2 = ucsc.rmsk.salmon_quant$analysis_set$panc_ctrl)
##
...
lapply(salmon_quant[names(salmon_quant) %in% c('bioIvt_panc_salmon', 'pittsburgh_ctrl_salmon')], 
             function(se) { colData(se$gxi) %>% as.data.frame() %>% remove_rownames()}) %>% bind_rows()

type <- 'gxi'
base_level <- 'ctrl'
sample_meta_in <- sample_meta_data$`2022.01.14_bioIVTAndPitts_meta_data.csv`
input_se <- salmon_quant$analysis_set$panc_ctrl 

colData(input_se[[type]]) %>% 
  as.data.frame() %>% 
  remove_rownames() 
  mutate(condition = relevel(as.factor(condition), base_level))
